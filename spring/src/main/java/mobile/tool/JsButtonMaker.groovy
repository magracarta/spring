...
def template = engine.createTemplate(new InputStreamReader(new FileInputStream(rootDir + '/src/main/java/mobile/tool/JSButtonDefine.template'), MakeUtil.CHAR_SET))

// 코드 조회 추출
def sql = Sql.newInstance(conf.url, conf.id, conf.passwd, conf.driverClassName)

def list = sql.rows(conf.jsQuery)
def binding = [
	jsPackageName : conf.jsPackageName,
	list : list,
]
// 파일명 확인
def fileName = rootDir + conf.jsPath + 'JSButtonDefine.java'
