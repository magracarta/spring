...
import mobile.factory.util.net.HttpUtil;
...
public class CollectionUtil {	/**
	 ...
	 * @return array
	 */
	public static String[] arrayMinusArray(String[] source, String[] dest) {
		if (ArrayUtils.isEmpty(source)) {
			source = new String[] { "" };
		}
		if (ArrayUtils.isEmpty(dest)) {
			dest = new String[] { "" };
		}

		String[] retArray = new String[source.length];

		for (int i = 0, n = source.length; i < n; i++) {
			if (!ArrayUtils.contains(dest, source[i])) {
				retArray[i] = source[i];
			} else {
				retArray[i] = "";
			}
		}
		return removeEmptyElement(retArray);
	}
	 ...
	 * 
	 * @param source
	 * @return 중복제거된 array (sort 안함)
	 */
	public static String[] arrayDupRemove(String[] source) {
		List<String> retList = new ArrayList<String>();

		String previousStr = "";
		for (String val : source) {
			if (!StringUtils.equals(previousStr, val)) {
				previousStr = val;
				retList.add(previousStr);
			}
		}

		Object[] objArray = retList.toArray();
		String[] retArray = new String[objArray.length];
		for (int i = 0, n = objArray.length; i < n; i++) {
	 ...
	 * @return
	 */
	public static List<Map<String, String>> null2Blank(List<Map<String, Object>> list) {
		if (list == null) {
			return null;
		}
		...
		for (Map<String, Object> map : list) {
			Map<String, String> retMap = null2Blank(map);
			retList.add(retMap);
		}
		return retList;
	}
	...
	/**
	 * 배열에 순서에 따라 맵을 만들어준다<br>
	 * src, numbering 의 사이즈는 같아야 한다.
	 * 
	 * @param src
	 * @param numbering
	 *            src의 순서에 따라 부여될 맵의 key
	 * @return 매핑 맵, 사이즈가 다를경우 빈 맵
	 */
	public static Map<String, String> arrayToMap(String[] src, String[] numbering) {
		Map<String, String> map = new HashMap<String, String>();
		if (src.length != numbering.length) {
			return map;
		}

		for (int i = 0, n = src.length; i < n; i++) {
			map.put(numbering[i], src[i]);
		}
		return map;
	}
	 ...
	 * @return
	 */
	public static List<String[]> arrayListToRemoveEmpty(List<String[]> list) {
		List<String[]> retList = new ArrayList<String[]>();
		for (int i = 0, n = list.size(); i < n; i++) {
			String[] row = list.get(i);

			String rowStr = Arrays.toString(row);
			rowStr = StringUtils.remove(rowStr, "[");
			rowStr = StringUtils.remove(rowStr, "]");
			rowStr = StringUtils.remove(rowStr, ",");

			if (StringUtils.isNotBlank(rowStr.trim())) {
				retList.add(row);
			}
		}
		return retList;
	...
	/**
	 * <pre>
	 * 	map<key, list<map<key, value>>> => map<key, list<string>> 로 변경
	 * </pre>
	 * 
	 * @param map
	 *            데이터 맵
	 ...
	 *            맵의 키로 들어갈 리스트
	 * @param valueKey
	 *            리스트 맵의 key 가 맵의 리스트의 값으로 들어갈 항목
	 * @return
	 */
	public static Map<String, List<String>> mapListToKeyMapListString(Map map, String[] listKey, String valueKey) {
		Map<String, List<String>> listMap = new HashMap<String, List<String>>();

		List<Map> rowList = null;
		for (String key : listKey) {
			List<String> list = new ArrayList<String>();
			rowList = (List<Map>) map.get(key);
			for (Map row : rowList) {
				list.add(row.get(valueKey).toString());
			}
	 ...
	 * @return
	 */
	public static Map<String, String> concatMap(List<Map> list) {
		Map<String, String> retMap = new HashMap<String, String>();
		for (Iterator<Map> it = list.iterator(); it.hasNext();) {
			...
			while (itMap.hasNext()) {
				...
				retMap.put(StringUtils.lowerCase(key), (String) map.get(key));
			}
		}
	...
	public static List<String> listMapToListString(List<Map> list, String key) {
		List<String> retList = new ArrayList<String>();
		for (Map row : list) {
			retList.add(row.get(key).toString());
		}
	 ...
	 * @return
	 */
	public static Map<String, Object> keyMap(Map<String, Object> map, String... field) {
		return keyMap(map, false, field);
	}
	...
	public static List convertUpperMapKey(List<Map<String, Object>> list) {
		if(list == null) {
			return null;
		}
		List<Map> retList = new ArrayList<>();
		for(Map row : list) {
			retList.add(convertUpperMapKey(row));
		}
		return retList;
	}
	
	/**
	 * 리스트 맵의 key 를 소문자로 변환
	 * @param list
	 * @return
	 */
	public static List convertLowerMapKey(List<Map<String, Object>> list) {
		if(list == null) {
			return null;
		}
		List<Map> retList = new ArrayList<>();
		for(Map row : list) {
			retList.add(convertLowerMapKey(row));
		}
		return retList;
	}
