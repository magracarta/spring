...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			parent.fnNonReadCnt($M.getValue('s_start_dt'), $M.getValue('s_end_dt'), "SYSTEM");
		});

		
		// 엔터키 이벤트
		function enter(fieldObj) {
			var field = ["s_paper_contents", "s_read_yn","s_confirm_yn"];
			$.each(field, function() {
		...
		function fnSearch(successFunc, isreload) {
			...
			$M.goNextPageAjax(this_page + "/search", $M.toGetParam(param), {method : 'get', loader : isLoading},
				function(result) {
					...
					if(result.success) {
						successFunc(result);
						parent.fnNonReadCnt($M.getValue('s_start_dt'), $M.getValue('s_end_dt'), "SYSYEM");
					};
				}
			);
		}
		...
		function fnDownloadExcel() {
			var exportProps = {
		  	...
		  	fnExportExcel(auiGrid, "시스템쪽지함", exportProps);
		}
		...
		function goRead() {
			...
			$M.goNextPageAjaxMsg(msg, "/mmyy/mmyy010201/read", $M.jsonArrayToForm(param), {method : 'post'},
				function(result) {
					if(result.success) {
						goSearch();
					};
							...
							<tr>
								...
								<td>
									<select class="form-control" id="s_read_yn" name="s_read_yn">
										...
										<option value="N" selected="selected">읽지않음</option>
