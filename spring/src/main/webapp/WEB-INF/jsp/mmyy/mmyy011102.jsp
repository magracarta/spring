...
<head>
	...
	function createAUIGrid() {
		...
		var columnLayout = [
			...
			},
			{
				...
				styleFunction : function(rowIndex, columnIndex, value, headerText, item, dataField) {
					if ('${page.fnc.F02009_001}' == 'Y' && item.aui_status_cd == 'C') {
						return "proc_style";
					} else {
						return null;
					}
		...
		AUIGrid.bind(auiGrid, "cellEditBegin", function( event ) {
			...
			if('${page.fnc.F02009_001}' != 'Y' || event.item.doc_buy_cd != '01') {
				if(event.dataField == "proc_dt" || event.dataField == "proc_mem_name" ) {
					return false; 
				}	
			}
		...
		AUIGrid.bind(auiGrid, "cellClick", function(event){
			if(event.dataField == "proc_dt" || event.dataField == "proc_mem_name" ) {
				...
				if (event.item.doc_buy_cd != '01') {
					setTimeout(function() {
						 AUIGrid.showToastMessage(auiGrid, event.rowIndex, event.columnIndex, "구분 : 구매 건만 입력가능합니다.");
					}, 1);
					return false; 
				}
			...
			if(event.dataField == "title") {
				...
				var popupOption = "";
				$M.goNextPage('/mmyy/mmyy011102p01', $M.toGetParam(param), {popupStatus : popupOption});
			}

			// 관리부, 구분 : 구매일경우에만 접근가능.
			if('${page.fnc.F02009_001}' == 'Y' && event.item.doc_buy_cd == '01' && event.item.appr_proc_status_cd == '05') {
	...
	// 지급취소
	function cancelProc(rowIndex) {

	}
	
	// 구매품의서 등록
	function goNew() {
		$M.goNextPage("/mmyy/mmyy01110201");
	}
	...
	function goSearch() {
		var param = {
			...
			"s_doc_buy_cd" : $M.getValue("s_doc_buy_cd"),
	...
	function fnExcelDownload() {
	  // 엑셀 내보내기 속성
	  var exportProps = {};
	  fnExportExcel(auiGrid, "품의서", exportProps);
	}
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>							
                                    ...
                                    <th>구분</th>
                                    <td>
										<select class="form-control" id="s_doc_buy_cd" name="s_doc_buy_cd">
