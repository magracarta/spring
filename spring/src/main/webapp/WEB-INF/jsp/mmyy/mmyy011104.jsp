...
// 			},
			...
			},
			{
				...
				styleFunction : function(rowIndex, columnIndex, value, headerText, item, dataField) {
					if ('${page.fnc.F02015_001}' == 'Y' && item.aui_status_cd == 'C') { 
						return "proc_style";
					} else {
						return null;
					}
		...
		AUIGrid.bind(auiGrid, "cellClick", function(event){
			...
			if(event.dataField == "title") {
				...
				var popupOption = "";
				$M.goNextPage('/mmyy/mmyy011104p01', $M.toGetParam(param), {popupStatus : popupOption});
			}
			
			// 관리부일경우에만 접근가능.
			if('${page.fnc.F02015_001}' == 'Y' && event.item.appr_proc_status_cd == '05') {
	...
	// 지출결의서 신청
	function goNew() {
		$M.goNextPage("/mmyy/mmyy01110401");
	}
...
// 			"s_doc_expense_pay_cd" : $M.getValue("s_doc_expense_pay_cd"),
	...
	function fnExcelDownload() {
	  // 엑셀 내보내기 속성
	  var exportProps = {};
	  fnExportExcel(auiGrid, "지출결의서", exportProps);
	}
	
	// 엔터키 이벤트
	function enter(fieldObj) {
		var field = ["s_mem_name", "s_title"];
		$.each(field, function() {
...
<!--                                     <th>사용구분</th> -->
<!--                                     <td> -->
<!-- 										<select class="form-control" id="s_doc_expense_pay_cd" name="s_doc_expense_pay_cd"> -->
<!-- 											<option value="">- 선택 -</option> -->
<%-- 											<c:forEach items="${codeMap['DOC_EXPENSE_PAY']}" var="item"> --%>
