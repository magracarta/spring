...
<head>
	...
	function goDataSearch() {
		...
		var httpMethod = postUrls.includes(url) ? "POST" : "GET";

		$M.goNextPageAjax(url, popGetParam, {dataType : 'html', method: httpMethod}, function(result) {
			var error = result.indexOf("{\"result_code\":\"500\"") > 0;
			if (error) {
				alert("해당 자료가 삭제되었거나, 유효하지 않습니다.");
			} else {
				if (httpMethod === "POST") {
					// 1초안에 두번 이상 POST 호출 시, 중복요청 필터링에 걸리므로 1초 대기 처리
					setTimeout(() => $M.goNextPage(url, popGetParam, {popupStatus : poppupOption, method : 'post'}), 1000);
				} else {
					$M.goNextPage(url, popGetParam, {popupStatus : poppupOption, method : 'post'});
				}
			}
		});
	}
