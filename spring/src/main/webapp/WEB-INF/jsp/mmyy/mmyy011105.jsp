...
<head>
	...
	$(document).ready(function() {
		console.log("${page.menu_add_cd_str}");
		// 그리드 생성
	...
	function createAUIGrid() {
		...
		var columnLayout = [
			...
			},
			{
				...
				labelFunction : function(rowIndex, columnIndex, value, headerText, item) {
					...
					var docMoneyManTypeName = item.doc_money_man_type_name;
					var docMoneyTypeName = item.doc_money_type_name;
					
					return docMoneyManTypeName + " " + docMoneyTypeName;
				}
			},
			{
				headerText: "경조일",
			...
			},
			{
				...
				styleFunction : function(rowIndex, columnIndex, value, headerText, item, dataField) {
					if ('${page.fnc.F02018_001}' == 'Y' && item.aui_status_cd == 'C') { 
						return "proc_style";
					} else {
						return null;
					}
		...
		AUIGrid.bind(auiGrid, "cellClick", function(event){
			...
			if(event.dataField == "apply_mem_name") {
				...
				var popupOption = "";
				$M.goNextPage('/mmyy/mmyy011105p01', $M.toGetParam(param), {popupStatus : popupOption});
			}
			
			// 관리부일경우에만 접근가능.
			if('${page.fnc.F02018_001}' == 'Y' && event.item.appr_proc_status_cd == '05') {
	...
	// 경조금 지급 신청서 등록
	function goNew() {
		$M.goNextPage("/mmyy/mmyy01110501");
	}
	...
	function goSearch() {
		var param = {
			...
			"s_doc_money_type_cd" : $M.getValue("s_doc_money_type_cd"),
	...
	function fnExcelDownload() {
	  // 엑셀 내보내기 속성
	  var exportProps = {};
	  fnExportExcel(auiGrid, "경조금지급신청서", exportProps);
	}
	...
	// 경조구분관리
	function goSetting(){
        $M.goNextPage('/mmyy/mmyy01110401p01', $M.toGetParam({}), {popupStatus : ""});
    }
	</script>
</head>
