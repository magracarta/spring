...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 공통 > 기준정보 > 부서권한관리 > null > null
-- 작성자 : 손광진
-- 최초 작성일 : 2020-01-13 15:18:14
-- 2022-12-09 jsk : erp3-2차 권한관리 수정
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	<script type="text/javascript">
						...
						onClick : function(event) {
							...
							if (confirm("해당 메뉴를 사용중인 모든 부서권한에 [일괄"+checked+"]하시겠습니까?") == true) {
								var param = {
									"org_code" : $M.getValue("org_code"),
									"menu_seq" : event.item.menu_seq,
									"menu_add_cd" : event.item.menu_add_cd,
				...
				function(result) {
					if(result.success) {
						...
						if(cellRowIndex != 0) {
							...
							if (getParam.org_code && getParam.menu_seq) {
								param.org_code = getParam.org_code;
								param.menu_seq = getParam.menu_seq;
								goSearchMenuDetail(param);
							}
						};
		...
		function goSearchMenuDetail(getParam) {
			...
			var param = {
					"s_sort_key" : "aa.sort_no",
					"s_sort_method" : "asc",
					"org_code" : getParam.org_code,
					"menu_seq" : getParam.menu_seq
			};
			$M.goNextPageAjax(this_page + "/" + getParam.org_code + "/" + getParam.menu_seq, $M.toGetParam(param), {method : 'get'},
				function(result) {
					if(result.success) {
						// 데이터 그리드 세팅
		...
		function goApply() {
			...
			$M.goNextPage('/comm/comm0114p01', $M.toGetParam(param), {popupStatus : poppupOption});
		}
		...
		function goSaveDetail() {
			...
			for(var i=0; i < addChangeData.length; i++) {
				menuAddCds.push(addChangeData[i].menu_add_cd);
			}
