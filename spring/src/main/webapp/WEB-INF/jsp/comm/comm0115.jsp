...
<head>
	...
	<script type="text/javascript">
		...
		function goSearchMenuDetail(getParam) {
			...
			$M.goNextPageAjax(this_page + "/" + getParam.job_auth_cd + "/" + getParam.menu_seq, "", {method : "get"},
				function(result) {
					if(result.success) {
						// 데이터 그리드 세팅
		...
		function getItemsByField() {
			...
			for(var i=0, len=activeItems.length; i<len; i++) {
				ids.push(activeItems[i].btn_seq); // btn_chk가 Y인 값만 저장 
			};
			return ids;
		}
		...
		function goSaveDetail() {
			...
			var param = {
				...
				"btn_seq_str" : $M.getArrStr(checkData),
