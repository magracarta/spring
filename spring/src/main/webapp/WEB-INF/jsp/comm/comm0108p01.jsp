	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			if(orgAuthCd.indexOf("#") != -1) {
				orgAuthCode = orgAuthCd.split('#');
				$M.setValue("org_auth_cd", orgAuthCode);
			} else {
			...
			if(jobAuthCd.indexOf("#") != -1) {
				jobAuthCode = jobAuthCd.split('#');
				$M.setValue("job_auth_cd", jobAuthCode);
			} else {
		...
		function goWebIdCheck() {
			...
			if ($M.getValue("web_id_chk") != "Y" ) {
				$M.goNextPageAjax(this_page+"/webIdCheck/"+$M.getValue("web_id")+ "/" + $M.getValue("mem_no"), '', {method : 'get'},
					function(result) {
			    		if(result.success) {
		...
		function goResiNoCheck() {
			...
			if ($M.getValue("resi_no_chk") != "Y" ) {
				...
				$M.goNextPageAjax(this_page+"/resiNoCheck/" + $M.getValue("mem_no"), $M.toGetParam(param), {method : 'post'},
					function(result) {
			    		if(result.success) {	    
		...
		function goHpNoCheck() {
			if($M.getValue("hp_no") == '' || $M.getValue("hp_no") == undefined) {
				...
				return;	
			} else if ($M.getValue("hp_no").length < 10) {
			...
			if ($M.getValue("hp_no_chk") != "Y" ) {	
				$M.goNextPageAjax(this_page+"/hpNoCheck/"+$M.getValue("hp_no")+ "/" + $M.getValue("mem_no"), '', {method : 'get'},
					function(result) {
			    		if(result.success) {
			    			$M.setValue("hp_no_chk","Y");
						...
						<table class="table-border mt5">
							...
							<tbody>
								<tr>
									...
									<td rowspan="5" class="text-center" >
										...
										<div class="profilephoto-item" onmouseover="javascript:fnDelBtnShow();" onmouseout="javascript:fnDelBtnHide();"   >
											<img id="profileImage" src="/file/${bean.pic_file_seq}" alt="프로필 사진" class="icon-profilephoto"  tabindex=0  />
									...
									<td>
										<input type="text" class="form-control essential-bg  width120px" value="${bean.kor_name}"   id="kor_name" name="kor_name" datatype="string" required="required" alt="직원명(한글)" maxlength="15"  >
									</td>	
									<th class="text-right">직원명(영문)</th>
									<td>
										<input type="text" class="form-control   width120px" value="${bean.eng_name}" id="eng_name" name="eng_name" datatype="string" maxlength="30" >
									</td>	
									<th class="text-right essential-item">아이디</th> <!-- 중복확인 체크되어있게하기 -->
									<td>
										<div class="form-row inline-pd widthfix">
											<div class="col  width100px">
												<input type="text" class="form-control essential-bg" value="${bean.web_id}" id="web_id"  name="web_id" required="required" alt="아이디" minlength="6" maxlength="30" >
									...
									<td>
										...
										<div class="form-row inline-pd">
											<div class="col width70px">
												<input type="text" id="resi_no1" name="resi_no1" class="form-control" minlength="6" maxlength="6" value="${fn:substring(bean.resi_no, 0, 6)}" datatype="int" >
											</div>
											<div class="col width16px text-center">~</div>
											<div class="col width70px">
												<input type="password" id="resi_no2" name="resi_no2" class="form-control" minlength="7" maxlength="7" value="${fn:substring(bean.resi_no, 6, 13)}" datatype="int"  >
											</div>
											<div class="col width60px" style="    padding-right: 0px;">
								...
								<tr>
									...
									<td>
										<div class="form-row inline-pd">
											<div class="col width140px">											
												<input type="text" class="form-control essential-bg" id="hp_no" name="hp_no"  value="${fn:replace(bean.hp_no, '-', '')}" format="phone" placeholder="숫자만 입력"  required="required" alt="핸드폰" >
											</div>
									...
									<td>
										<div class="form-row inline-pd">
											...
											<div class="col-6">
												<input type="text" class="form-control essential-bg" id="emergency_contact_phone_no" name="emergency_contact_phone_no"  placeholder="-없이 숫자만"  value="${bean.emergency_contact_phone_no}" datatype="int" minlength="9"  maxlength="12" required="required" alt="비상연락처" >
											</div>
										</div>
									</td>		
									<th class="text-right">이메일</th>
									<td>
										<input type="text" class="form-control width140px" value="${bean.email}"  id="email" name="email"  format="email" >
									</td>							
								</tr>
								<tr>
									...
									<td>
										<input class="form-control" style="width: 99%;"type="text" id="org_code" name="org_code" easyui="combogrid"
											easyuiname="orgAllDepthList" panelwidth="350" textfield="path_org_name" multi="N" idfield="org_code" value="${bean.org_code}"/>
									</td>		
									<th class="text-right">전화(사무실)</th>
									<td>
										<input type="text" class="form-control  width140px" value="${bean.office_tel_no}"  id="office_tel_no" name="office_tel_no"  placeholder="-없이 숫자만" datatype="int" minlength="9"  maxlength="12">
									</td>	
									<th class="text-right">전화(팩스)</th>
									<td>
										<input type="text" class="form-control  width140px" value="${bean.office_fax_no}"  id="office_fax_no" name="office_fax_no" placeholder="-없이 숫자만" datatype="int" minlength="9"  maxlength="12" >
									</td>						
								</tr>
								<tr>
									<th class="text-right essential-item">부서권한</th>
									<td >
										<input class="form-control" style="width: 99%;"type="text" id="org_auth_cd" name="org_auth_cd" easyui="combogrid"
											easyuiname="org2DepthList" panelwidth="350" textfield="org_name" multi="Y" idfield="org_code"/>
									</td>		
									...
									<td colspan="3">
										<input class="form-control" style="width:400px" type="text" id="job_auth_cd" name="job_auth_cd" easyui="combogrid"
											easyuiname="jobAuthList" panelwidth="400" textfield="code_name" multi="Y"  idfield="code_value"  />
									</td>					
								</tr>
								<tr>
									...
									<td>									
										<select class="form-control essential-bg" id="work_status_cd" name="work_status_cd"   required="required" alt="재직구분"  >
											...
											<c:forEach items="${codeMap['WORK_STATUS']}" var="item">
												<option value="${item.code_value}" ${item.code_value == bean.work_status_cd ? 'selected' : '' }>
													${item.code_name}
												</option>
											</c:forEach>	
										</select>									
											...
											<div class="col-12">
												<select class="form-control essential-bg" id="grade_cd" name="grade_cd" required="required" alt="직급" >
													...
													<c:forEach items="${codeMap['GRADE']}" var="item">
														<option value="${item.code_value}" ${item.code_value == bean.grade_cd ? 'selected' : '' }>
															${item.code_name}
														</option>
													</c:forEach>
												</select>
									...
									<td colspan="3" rowspan="3">
										<div class="form-row inline-pd mb7 widthfix">
											<div class="col width100px">
												<input type="text" class="form-control essential-bg" value="${bean.home_post_no }"  id="home_post_no" name="home_post_no" readonly="readonly"  required="required"   alt="자택주소 입력"  >
											</div>
											<div class="col width60px">
										...
										<div class="form-row inline-pd mb7">
											<div class="col-12">
												<input type="text" class="form-control essential-bg" value="${bean.home_addr1  }"  id="home_addr1" name="home_addr1" readonly="readonly"  required="required"   alt="자택주소 입력"  >
											</div>
										</div>
										<div class="form-row inline-pd">
											<div class="col-12">
								...
								<tr>
									...
									<td>
										<div class="input-group  width120px" id="retireField" name="retireField">
											<input type="text" id="retire_dt" name="retire_dt" dateFormat="yyyy-MM-dd" class="form-control border-right-0 calDate" value="${bean.retire_dt}"  readonly="readonly" alt="퇴직일" >
										</div>
									</td>	
									...
									<td>
										<select class="form-control essential-bg" id="job_cd" name="job_cd"    required="required" alt="직책"  >
											...
											<c:forEach items="${codeMap['JOB']}" var="item">
												<option value="${item.code_value}" ${item.code_value == bean.job_cd ? 'selected' : '' }>
													${item.code_name}
												</option>
											</c:forEach>	
										</select>
								...
								<tr>
									...
									<td colspan="3">
										<input type="text" class="form-control" id="cert_company_opinion" name="cert_company_opinion" readonly="readonly" value="${bean.cert_company_opinion}" alt="퇴직 시 회사의견" >
									</td>	
								</tr>							
							</tbody>
						</table>
...
<!-- /기본정보 -->	
					<div class="btn-group mt10">
						<div class="right">
							<button type="button" onclick="goSave();" class="btn btn-info" style="width: 60px;">저장</button>
							<button type="button" onclick="fnClose();" class="btn btn-info" style="width: 60px;">닫기</button>
						</div>
					</div>
				</div>						
			</div>		
