...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 공통팝업 > 업무DB > 업무DB팝업 > null > 메이커 별 모델 별 목록
-- 작성자 : 박예진
    ...
    <script type="text/javascript">
		...
		function fnAddFolder(lineDriveSeq) {
			...
			if(nameYn) {
				...
				str += '</div>';
				str += '<div class="info">';
				str += '<input type="text" class="add_folder_name" id="resource_name_' + listIndex + '" placeholder="새폴더" onfocusout="javascript:fnChangeName(' + listIndex + ', this.value, \'' + lineDriveSeq + '\', \'folder_name\');">';
				str += '</div>';
				str += '</a>';
		...
		// 폴더 클릭 시 장비리스트 조회
		function goMchListPage(lineDriveSeq) {
			var param = {
					"line_drive_seq" : lineDriveSeq,
			...
			$M.goNextPage('/workDb/workDb0103', $M.toGetParam(param));
		}
...
//      		var param = {
//      		}
//      		$M.goNextPage('/workDb/workDb0101', $M.toGetParam(param));
//      	}
...
<!-- 리스트 -->
            <div class="folder-items2 mt20">
            	<c:forEach var="item" items="${list}">
            		<c:if test="${item.sale_yn eq 'Y'}">
				 		<c:set var="saleYn" value="b-folder"/>
                    </c:if>
                    <c:if test="${item.sale_yn ne 'Y'}">
            		...
            		<a class="folder-item2">
	                    <c:if test="${item.file_type eq 'folder'}">
	                    	...
	                    	<div class="thumb" onclick="javascript:goMchListPage('${item.line_drive_seq}');">
	                    </c:if>
	                    <c:if test="${item.file_type ne 'folder'}">
				 		<c:set var="resourceKey" value="${item.up_resource_key}"/>
	                        ...
	                        <div class="num">${item.child_folder_count}</div>
				...
				<c:if test="${bean.modify_yn eq 'Y'}">
	                <a class="folder-item2" id="addFolder">
	                    ...
	                    <div class="info dpn"> <!-- 폴더 추가 아이콘 클릭시, dpn 클래스 삭제 -->
	                        <input type="text" style="width: 100%;" class="folder_name">
	                    </div>
	                </a>
                </c:if>
            </div>
