...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 공통팝업 > 업무DB > 업무DB팝업 > null > 모델 별 부서 별 폴더 목록
-- 작성자 : 박예진
    ...
    <script type="text/javascript">
		...
		function fnAddFolder(lineDriveSeq) {
			...
			if(nameYn) {
				...
				str += '</div>';
				str += '<div class="info">';
				str += '<input type="text" id="resource_name_' + listIndex + '" placeholder="새폴더" onfocusout="javascript:fnChangeName(' + listIndex + ', this.value, \'' + lineDriveSeq + '\', \'folder_name_' + lineDriveSeq + '\');">';
				str += '</div>';
				str += '</a>';
		...
		// 폴더 클릭 시 폴더 리스트 조회 
		function goMchSubListPage(lineDriveSeq) {
			var param = {
					"line_drive_seq" : lineDriveSeq,
...
<!-- 시리즈1 -->
			<c:forEach var="mchList" items="${mchList}">
          	 	<c:if test="${mchList.sale_yn eq 'Y'}">
		 			<c:set var="saleYn" value="b-folder"/>
                </c:if>
                <c:if test="${mchList.sale_yn ne 'Y'}">
		 			<c:set var="saleYn" value="g-folder"/>
                </c:if>
	            <div class="folder-items2 folder-step">
	                <div class="folder-upper">
	                    <a class="folder-item2">
	                    	<c:if test="${mchList.file_type eq 'folder'}">
					 			<c:set var="resourceKey" value="${mchList.resource_key}"/>
		                    	<div class="thumb" onclick="javascript:goMchSubListPage('${mchList.line_drive_seq}');">
		                    </c:if>
		                    <c:if test="${mchList.file_type ne 'folder'}">
					 		<c:set var="resourceKey" value="${mchList.up_resource_key}"/>
		                    	<div class="thumb" onclick="javascript:goLineworks('${mchList.up_resource_key}');">
		                    </c:if>
		                        <div class="hover"></div>
		                        <div class="btns">
		                        	<c:if test="${mchList.modify_yn eq 'Y'}">
				                        <button type="button" class="btn btn-icon btn-light" onclick="javascript:fnChangeName('${mchList.line_drive_seq}', '${mchList.line_name}', '', 'folder_high_name');"><i class="material-iconscreate text-default"></i></button>
		                            </c:if>
		                        ...
		                        <div class="num">${mchList.child_folder_count}</div>
		                        <span class="icon-folder ${saleYn}"></span>
		                    </div>
<%-- 	                        <c:if test="${not empty mchList.first_sale_dt|| not empty mchList.last_sale_dt}"> --%>
	                        <div class="fromto">
	                            <div class="from">
	                                <div class="tit">From</div>
	                                <div>${mchList.first_sale_dt}</div>
	                            </div>
	                            <div class="to">
	                                <div class="tit">To</div>
	                                <div>${mchList.last_sale_dt}</div>
	                            </div>
	                        </div>
<%-- 	                       	</c:if> --%>
		                    <div class="info folder_high_name" style="font-weight:bold;" id="${mchList.line_drive_seq}">
		                   		${mchList.line_name}
		                    </div>
	                    </a>
	                </div>
	                <div class="folder-lower">
		        		<c:forEach var="mchMap" items="${mchMap}">
		        			<c:if test="${mchMap.key eq mchList.line_drive_seq}">
		        				<c:forEach var="subList" items="${mchMap.value}">
			        				<a class="folder-item2">
					                    ...
					                    <c:if test="${subList.file_type ne 'folder'}">
								 		<c:set var="resourceKey" value="${subList.up_resource_key}"/>
				                            ...
				                            <div class="hover"></div>
				                            <div class="btns">
				                                <c:if test="${subList.modify_yn eq 'Y'}"> <!-- 같은 상위에서 중복체크하도록 추가해야함 -->
					                                <button type="button" class="btn btn-icon btn-light" onclick="javascript:fnChangeName('${subList.line_drive_seq}', '${subList.line_name}', '', 'folder_name_${mchList.line_drive_seq}');"><i class="material-iconscreate text-default"></i></button>
					                            </c:if>
					                            	<button type="button" class="btn btn-icon btn-light" onclick="javascript:goLineworks('${resourceKey}');"><i class="material-iconslink text-default"></i></button>
				                            </div>
				                            <div class="num">${subList.child_folder_count}</div>
				                            <span class="icon-folder y-folder"></span>
				                        </div>
				                        <div class="info folder_name_${mchList.line_drive_seq}" id="${subList.line_drive_seq}">
				                        	${subList.line_name}
				                        </div>
				                    </a>
		        				</c:forEach>
	        				<c:if test="${bean.modify_yn eq 'Y'}">
				                <a class="folder-item2" id="addFolder_${mchList.line_drive_seq}">
				                    <div class="thumb" onclick="javascript:fnAddFolder('${mchList.line_drive_seq}');">  <!-- 똑같은 폴더 아이콘 추가하고, 밑에 input 추가하기 -->                     
				                        <span class="icon-folder folder-add"></span>
				                    </div>
				                    <div class="info dpn"> <!-- 폴더 추가 아이콘 클릭시, dpn 클래스 삭제 -->
				                        <input type="text" style="width: 100%;" class="folder_name_${mchList.line_drive_seq}">
				                    </div>
				                </a>
			                </c:if>
		        			</c:if>
