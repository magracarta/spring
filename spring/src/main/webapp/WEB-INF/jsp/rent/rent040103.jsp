...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 렌탈 > 렌탈현황 > 렌탈장비현황 > 모델별 가동시간별 장비현황 > null
-- 작성자 : 김태훈
...
<head>
	...
	<%-- 여기에 스크립트 넣어주세요. --%>
		...
		function createAUIGrid() {
			...
			<c:forEach items="${array}" var="item" varStatus="loop">
				var header = "~${item}";
				<c:if test="${loop.last}">
					header = "${item-1}"+"초과";
				</c:if>
			...
			AUIGrid.bind(auiGrid, "cellClick", function(event) {
				if (event.dataField != "maker_name" && event.dataField != "machine_name"
					...
					var params = {
						...
						op_hour : event.dataField != "total" ? event.dataField : ""
					};
		...
		function fnDownloadExcel() {
			var exportProps = {};
			fnExportExcel(auiGrid, "모델별 가동시간별 장비현황", exportProps);
	    }
