...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 렌탈 > 렌탈운영 > 렌탈장비 출고/회수현황 > null > 렌탈연장/회수처리
-- 작성자 : 김태훈
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			/*if("${inputParam.assign_same_yn}" != "Y"){
				$("#_goRentalCancel,#_goExtendPopup,#_goExtend,#_goReturnEarly,#_goApprovalEnd,#_goReturn,#_goApprovalEndCancel").hide();
			}*/
			// 종결 , 정산처리 버튼 업무관리자 , 배정자, 서비스관리 (서비스관리에게만 종결취소 버튼 노출) F00968_002
			<%--if ("${page.fnc.F00968_002}" != "Y" && "${assign_same_yn}" != "Y") {--%>
			if ("${rental_assign_mem_no}" == "") {
				$("#_goRentalCancel,#_goExtendPopup,#_goExtend,#_goReturnEarly,#_goApprovalEnd,#_goReturn,#_goSale,#_goModify,#_goApprovalEndCancel").hide();
			}else {
				<%--if ("${page.fnc.F00968_002}" != "Y" && "${rental_assign_mem_no}" != "${SecureUser.mem_no}") {--%>
				if ("${page.fnc.F00968_002}" != "Y" && "${rental_assign_mem_no}" != "${SecureUser.mem_no}") {
					$("#_goRentalCancel,#_goExtendPopup,#_goExtend,#_goReturnEarly,#_goApprovalEnd,#_goReturn,#_goSale,#_goModify").hide();
				}
		...
		function fnCalc() {
			...
			var sumAttachAmt = 0;
			var gridData;
			if(${inputParam.req_extend eq 'Y'}) {
				gridData = AUIGrid.getCheckedRowItemsAll(auiGrid);
			} else {
		...
		function goSaleForEarlyReturn() {
			...
			// var amt = $M.toNum($M.getValue("return_amt"));
			// if (amt > 0) {
			// 	amt = "-" + amt;
			// }
		...
		function goRentalCancel() {
			if (confirm("렌탈을 취소하시겠습니까?") == false) {
				return false;
			}
			// 연장안됐을때
			var callback = "goRentalCancelForExtendAfterSaleCancel";
			// 연장되지않고, 마지막자료
			if ("${rent.last_yn}" == "Y") {
				if ("${rent.inout_doc_no[0]}" != "") {
					...
					// 매출삭제된 자료
					goRentalCancelForExtendAfterSaleCancel("");
				}
		...
		// 렌탈취소 - 매출처리삭제 콜백(연장된 자료)
		function goRentalCancelForExtendAfterSaleCancel(row) {
			var param = {
				rental_doc_no : "${rent.rental_doc_no}"
			}
			$M.goNextPageAjax("/rent/rent0102p01/cancelRental", $M.toGetParam(param), {method : 'POST'},
					function(result) {
			    		if(result.success) {
			    			...
			    			if (result.goUrl == "/rent/rent0101p01") {
			    				fnClose();
			    			}
						}
					}
		...
		function goSale() {
			...
			// 매출자료 했는지 체크
			$M.goNextPageAjax("/rent/rent0102p02/check", $M.toGetParam(param), {method : 'GET'},
					function(result) {
			    		if(result.success) {
			    			if (result.inout_proc_yn == "Y") {
			    				alert("이미 매출처리된 자료입니다.");
			    				location.reload();
			    			} else {
			    				// 안했으면 팝업호출
		...
		function goAttachPopup() {
			...
			// 고객렌탈일떄는 관리센터의 어태치만 조회
	     	var params = {
	     		...
	     		not_rental_attach_no : "${inputParam.req_extend}" == "Y" ? $M.getArrStr(rows, {key : 'rental_attach_no'}) : "",
		    };
		...
		function fnSetAttach(row) {
			...
			if(row != null) {
				...
				var partArrStr = $M.getArrStr(list, {key : 'part_no'});

				for(var i=0; i<row.length; i++) {
					if(partArrStr.includes(row[i].part_no)) {
						alert(row[i].part_no + "은 이미 존재합니다.\n어태치먼트 교체는 모바일에서 진행해주세요.");
						return;
					}
				}
				for(var i=0; i<row.length; i++) {
					...
					item.attach_name = row[i].attach_part_name;
					item.qty = 1;
		...
		function fnInit() {
			...
			if ("${rent.out_dt}" == "") {
				...
				$(".out_div input").prop("readonly", true);
			...
			// 렌탈연장일 경우 연장처리와 닫기만(연장하고 있는 중에 갑자기 회수 방지..)
			if ("${inputParam.req_extend}" == "Y") {
			...
			} else { // 렌탈연장이 아닐 경우 렌탈연장,회수,조기회수
				$("#rental_ed_dt").attr("disabled", true);
				$("#discount_amt").attr("disabled", true);
				$("#_goExtend").css("display", "none");
				/* $("button[onclick='javascript:goReturnEarly();']").css("display", "none");
				$("button[onclick='javascript:goReturn();']").css("display", "none"); */
			}
			if ("N" == "${rent.last_yn}") {
				$("#_goExtendPopup").css("display", "none");
				$("#_goReturnEarly").css("display", "none");
			...
			<c:if test="${rent.paper_file_seq ne 0 and inputParam.req_extend ne 'Y'}">
		...
		function fnSetDayCnt() {
	    	if ($M.getValue("rental_st_dt") == "" || $M.getValue("rental_ed_dt") == "") {
	        	...
	        	<c:if test="${rent.inout_proc_yn ne 'Y'}">
	        		getMachinePrice();
	        	</c:if>
	        	<c:if test="${inputParam.req_extend eq 'Y'}">
	        		getMachinePrice();
	        	</c:if>
	    	}
	    }
...
<%--								AUIGrid.showToastMessage(auiGrid, event.rowIndex, 8, "기본어태치먼트는 삭제할 수 없습니다.");--%>
<%--							} else {--%>
...
<%--									AUIGrid.updateRow(auiGrid, {cmd : "U"}, event.rowIndex);--%>
<%--									getAttachPrice();--%>
<%--								}--%>
<%--							}--%>
<%--						}--%>
...
<%--				</c:if>--%>
			...
			<c:if test="${inputParam.req_extend eq 'Y'}">
				<c:if test="${not empty attach}">
					...
					for(var i = 0; i < gridData.length; i++) {
						partArr.push(gridData[i].part_no);
					}
					AUIGrid.setCheckedRowsByValue(auiGrid, "part_no", partArr);
		...
		function fnReturnValidation() {
			...
			// (SR : 15591 - 황빛찬) 회수 사동시간이 더 작아도 수정 가능하게 적용
		...
		function goProcess(control) {
			if ("${rent.inout_proc_yn}" != "Y") {
				alert("매출처리되지 않은 자료입니다.");
				return false;
			}
			var selectiveParam;
			var param = {
			...
			if (control == "R") {
				...
				// } else {
					...
					selectiveParam = $M.toGetParam(param);
				// }
			} else if (control == "E") {
				msg = "연장처리하시겠습니까?";
				$M.setValue("mode", control);
				if (confirm(msg) == false) {
					return false;
				}

				<%--if(('${modusignMap.file_seq}' == '' || '${modusignMap.file_seq}' == '0') && $M.getValue("paper_file_seq") == '') {--%>
				<%--	alert('전자서명이 완료되거나 종이계약서 업로드 시 연장처리가 가능합니다.');--%>
				<%--	return false;--%>
				<%--}--%>

				frm = $M.toValueForm(frm);
				/* var selectiveParam = fnChangeGridDataToForm(auiGrid);
				$M.copyForm(selectiveParam, frm); */
				selectiveParam = fnCheckedGridDataToForm(auiGrid);
				// var isDeleted = false;
				// for (var i = 0; i < auiList.length; ++i) {
				// 	if (auiList[i].cmd != "D") {
				// 		AUIGrid.updateRow(auiGrid, {"cmd" : "C"}, AUIGrid.rowIdToIndex(auiGrid, auiList[i]._$uid));
				// 	}
				// 	if (auiList[i].cmd == "D") {
				// 		isDeleted = true;
				// 	}
				...
				// var concatList = AUIGrid.exportToObject(auiGrid);
				// selectiveParam = fnGridDataToForm(concatCols, concatList);
				$M.copyForm(selectiveParam, frm);
			} else if(control == 'ME') {
				frm = $M.toValueForm(frm);
				selectiveParam = fnCheckedGridDataToForm(auiGrid);
			...
			$M.goNextPageAjax(this_page, selectiveParam, {method : 'POST'},
					function(result) {
			    		if(result.success) {
			    			if(result.rental_doc_no == null) {
			    				/* alert("마지막 자료에서 시도해주세요."); */
			    				if (control == "R") {
			    					alert("처리가 완료되었습니다.");
			    					location.reload();
			    				}
			    				...
			    				alert("처리가 완료되었습니다.");
				    			if (control == "E" || control == "ME") {
				    				// 새로운 연장번호로 매출처리 호출
				    				var param = {
				    					rental_doc_no : result.rental_doc_no
				    				}
									// 매출처리 시 모두싸인이 완료되거나 종이계약서가 업로드되어야하므로 주석처리
				    				// openInoutProcPanel("fnGetResultInoutProc", $M.toGetParam(param));
				    			}
				    			var param = {
				    				rental_machine_no : $M.getValue("rental_machine_no"),
				    				rental_doc_no : result.rental_doc_no
				    			...
				    			window.location.href = this_page+"?"+$M.toGetParam(param);
			    			}
						}
					}
				);
		...
		function goReturnEarlyPopup() {
			...
			var rentalAttachNoStr = [];
			var list = AUIGrid.getGridData(auiGrid);
			if (list.length != 0) {
				rentalAttachNoStr = $M.getArrStr(list, {key : "rental_attach_no"});
				console.log(rentalAttachNoStr);
			}
			var params = {
				rental_doc_no : $M.getValue("rental_doc_no"),
...
// 			if ($M.toNum("${inputParam.s_end_dt}") > $M.toNum("${rent.rental_ed_dt}")) {
// 				console.log("${inputParam.s_end_dt}", $M.getValue("rental_ed_dt"));
// 				alert("연장 가능한 날짜가 지났습니다.");
// 				return false;
// 			}
		...
		function goApprovalEnd() {
			...
			$M.goNextPageAjaxMsg("종결처리 하시겠습니까?", this_page + "/end", $M.toGetParam(param), {method : 'POST'},
				function(result) {
					if(result.success) {
						alert("정상 처리되었습니다.");
						fnReload();
		...
		function goApprovalEndCancel() {
			...
			$M.goNextPageAjaxMsg("종결취소처리 하시겠습니까?", this_page + "/end/cancel", $M.toGetParam(param), {method : 'POST'},
					function(result) {
						if(result.success) {
							alert("정상 처리되었습니다.");
							fnReload();
		...
		function goChangeSave() {
			...
			$M.goNextPageAjaxMsg("회수 시 가동시간을 수정 하시겠습니까?",'/rent/rent0102p01/changeOpHour', $M.toGetParam(param), {method : 'POST'},
					function(result) {
						if(result.success) {
							location.reload();
						}
		...
		function sendModusignPanel() {
			...
			var params = {
				...
				"confirm_msg" : '${inputParam.req_extend}' == 'Y' ? "발송 시 연장처리 후 발송처리됩니다.\n발송하시겠습니까?" : "저장된 내용으로 싸인 문서가 발송됩니다.\n내용 변경시 저장 및 싸인취소 후 다시 재진행하셔야 합니다.",
		...
		function sendModusignAfterSave(data) {
			if('${inputParam.req_extend}' == 'Y') {
				...
				$M.setValue("hp_no", data.modusign_send_value);
				$M.setValue("send_email", data.modusign_send_value);

				$M.setValue("send_modusign_yn", "Y");
				$M.setValue("mode", "ME");
				goProcess('ME');
			} else {
				var param = {
		...
		function goSavePaperFile() {
			...
			$M.goNextPageAjaxMsg("종이계약서를 저장하시겠습니까?", "/rent/rent0102p01/paper/save", $M.toGetParam(param), {method : 'POST'},
					function(result) {
						if(result.success) {
							alert("처리가 완료되었습니다.");
						}
...
<input type="hidden" name="last_rental_ed_dt" value="${rent.rental_ed_dt}">
					...
					<div class="title-wrap approval-left">
						...
						<div class="right">
							<span class="condition-item">상태 :
								<c:choose>
									<c:when test="${not empty rent.return_dt and rent.rental_status_cd eq '05' }">종결</c:when>
									<c:when test="${not empty rent.return_dt and rent.rental_status_cd ne '05' }">미정산</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test="${not empty inputParam.req_extend and inputParam.req_extend eq 'Y'}">연장신청</c:when>
											<c:when test="${'Y' eq rent.extend_yn and empty inputParam.req_extend}">연장</c:when>
							...
							<tr>
								...
								<td colspan="5">
									<div class="row">
										...
										<c:if test="${inputParam.req_extend ne 'Y'}">
										<div class="col-6">
											<input type="text" class="form-control" readonly="readonly" id="addr1" name="addr1" value="${rent.addr1 }">
										</div>
										<div class="col-6">
...
<!-- 렌탈정보 -->
					...
					<table class="table-border mt5">
						...
						<tbody>
								...
								<td colspan="3">
									<div class="form-row inline-pd widthfix">
										<div class="col width120px">
											<input type="text" class="form-control" readonly="readonly" value="${rent.rental_doc_no}" style="display: inline-block;">
										</div>
										<c:choose>
											<c:when test="${empty rent.inout_proc_yn or rent.inout_proc_yn ne 'Y'}">
												<span style="color: red">매출처리 필요</span>
											</c:when>
											<c:when test="${empty inputParam.req_extend or inputParam.req_extend ne 'Y'}">
							...
							<tr>
								<th class="text-right">렌탈기간</th>
								<td colspan="7">
									<div class="form-row inline-pd widthfix">
										<div class="col width110px">
											<div class="input-group">
												<input type="text" class="form-control border-right-0 calDate" id="rental_st_dt" name="rental_st_dt" dateFormat="yyyy-MM-dd" alt="렌탈 시작일" value="${rent.rental_st_dt }" required="required" disabled="disabled">
											</div>
										</div>
										<div class="col width16px text-center">~</div>
										<div class="col width120px">
											<div class="input-group">
												<input type="text" class="form-control border-right-0 calDate" id="rental_ed_dt" name="rental_ed_dt" dateFormat="yyyy-MM-dd" alt="렌탈 종료일" value="${rent.rental_ed_dt }" onchange="fnSetDayCnt()" required="required">
											</div>
										</div>
										...
										<div class="col width60px text-right ml15">
											연장회차
										</div>
										<div class="col width50px text-right">
											<input type="text" class="form-control" readonly="readonly" value="${rent.extend_cnt }">
										</div>
										<div class="col width16px">
											차
										</div>
							...
							<tr class="out_div">
								...
								<td colspan="3">
									<select class="form-control width130px rb inline" id="rental_delivery_cd" name="rental_delivery_cd" alt="인도방법" required="required" disabled="disabled">
										<option value="">- 선택 -</option>
									...
									<div style="display: inline-block; vertical-align: middle; margin-left: 5px;">
										<div class="two_way_yn" <c:if test="${rent.rental_delivery_cd != '03'}">style="display: none;"</c:if>>
											<div class="form-check form-check-inline">
												<input class="form-check-input" type="radio" id="two_way_yn_n" name="two_way_yn" value="N" <c:if test="${rent.two_way_yn == 'N'}">checked="checked"</c:if> disabled="disabled">
												<label class="form-check-label" for="two_way_yn_n">편도</label>
											</div>
											<div class="form-check form-check-inline">
												<input class="form-check-input" type="radio" id="two_way_yn_y" name="two_way_yn" value="Y" <c:if test="${rent.two_way_yn == 'Y'}">checked="checked"</c:if> disabled="disabled">
								...
								<td colspan="3">
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="checkbox" name="contract_make_yn_check" id="contract_make_yn_check" disabled="disabled" value="Y" <c:if test="${rent.contract_make_yn == 'Y'}">checked="checked"</c:if>>
										<label class="form-check-label" for="contract_make_yn_check">계약서작성</label>
									</div>
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="checkbox" name="id_copy_yn_check" id="id_copy_yn_check" disabled="disabled" value="Y" <c:if test="${rent.id_copy_yn == 'Y'}">checked="checked"</c:if>>
									...
									<div class="form-row inline-pd">
                                        ...
                                        <!-- <div class="col-auto pdl5">
                                            <button type="button" class="btn btn-primary-gra full"
                                                    onclick="javascript:openSearchAddrPanel('fnSetArrival1Addr');" readonly="readonly">주소찾기
                                            </button>
                                        </div> -->
                                        ...
                                        <div class="col-4">
                                            <input type="text" class="form-control" id="delivery_addr2"
                                                   name="delivery_addr2" value="${rent.delivery_addr2}" readonly="readonly">
                                        </div>
                                    </div>
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<div class="form-row inline-pd widthfix">
										<div class="col width100px">
											<input type="text" class="form-control text-right" id="transport_amt" name="transport_amt" format="decimal" value="${inputParam.req_extend eq 'Y' ? 0 : rent.transport_amt}" <c:if test="${not empty rent.out_dt}">disabled="disabled"</c:if>>
										</div>
							...
							<tr>
								...
								<td colspan="3">
									<textarea class="form-control" style="min-height: 70px; height: 100%" id="remark" name="remark" maxlength="100">${rent.remark }</textarea>
								</td>
										   ...
										   <c:forEach items="${shareList}" var="item">
										      <tr>
										         ...
										         <td>
										         	...
										         	<div class="input-group">
<%--														<select class="form-control width130px rb inline" id="profit_org_code_${item.code_value}" name="profit_org_code_${item.code_value}" required="required" alt="${item.code_name}" <c:if test="${not empty item.org_code}">disabled</c:if>>--%>
<%--															<option value="">- 선택 -</option>--%>
...
<%--								<input type="text" class="form-control width24px cInput" id="big_bucket_cnt" name="big_bucket_cnt" alt="대버켓 숫자" value="${rent.big_bucket_cnt}" datatype="int" <c:if test="${inputParam.req_extend ne 'Y'}">disabled="disabled"</c:if>>--%>
<%--							</div>--%>
<%--							<div class="vl">|</div>--%>
<%--							<div>--%>
<%--								<span style="margin-right: 3px; line-height: 2"> 중</span>--%>
<%--								<input type="text" class="form-control width24px cInput" id="mid_bucket_cnt" name="mid_bucket_cnt" alt="중버켓 숫자" value="${rent.mid_bucket_cnt}" datatype="int" <c:if test="${inputParam.req_extend ne 'Y'}">disabled="disabled"</c:if>>--%>
<%--							</div>--%>
<%--							<div class="vl">|</div>--%>
<%--							<div>--%>
<%--								<span style="margin-right: 3px; line-height: 2"> 소</span>--%>
<%--								<input type="text" class="form-control width24px cInput" id="sml_bucket_cnt" name="sml_bucket_cnt" alt="소버켓 숫자" value="${rent.sml_bucket_cnt}" datatype="int" <c:if test="${inputParam.req_extend ne 'Y'}">disabled="disabled"</c:if>>--%>
<%--							</div>--%>
<%--							<div class="vl">|</div>--%>
<%--							<div>--%>
<%--								<span style="margin-right: 3px; line-height: 2"> 키</span>--%>
<%--								<input type="text" class="form-control width24px cInput" id="key_cnt" name="key_cnt" alt="키 숫자" value="${rent.key_cnt}" datatype="int" <c:if test="${inputParam.req_extend ne 'Y'}">disabled="disabled"</c:if>> --%>
<%--							</div>--%>
<%--						</div>--%>
						<c:if test="${inputParam.req_extend eq 'Y'}">
							<button type="button" class="btn btn-primary-gra" onclick="javascript:goAttachPopup();">어테치먼트 추가</button>
						</c:if>
						<c:if test="${inputParam.req_extend ne 'Y' and empty rent.return_dt}">
...
<%--								<div class="input-group">--%>
<%--									<c:if test="${(inputParam.req_extend eq 'Y' or empty rent.modusign_id) and page.add.MODUSIGN_YN eq 'Y'}">--%>
...
<%--									</c:if>--%>
<%--									<c:if test="${inputParam.req_extend ne 'Y' and not empty rent.modusign_id and modusignMap.complete_yn eq 'N'}">--%>
<%--										<button type="button" class="btn btn-primary-gra"  onclick="javascript:void();" disabled>${modusignMap.modusign_status_label}</button>--%>
<%--										<button type="button" class="btn btn-primary-gra ml5" onclick="javascript:sendModusignCancel()">싸인취소</button>--%>
<%--									</c:if>--%>
<%--									<c:if test="${inputParam.req_extend ne 'Y' and modusignMap.file_seq ne 0}">--%>
...
<%--										<input type="text" readonly="readonly" value="${early.use_amt }" id="use_amt" name="use_amt" format="decimal" class="form-control text-right" placeholder="사용금액" style="display: inline-block; width: 100px"><span style="display: inline-block;margin-left: 5px">원</span>--%>
								...
								<tr>
									...
									<td colspan="3">
										...
										<input type="text" ${readOnlyAttr} class="form-control text-right" id="return_amt" name="return_amt" value="${early.return_amt }" format="minusNum" placeholder="환불금액" style="display: inline-block; width: 100px">
								...
								<tr>
									...
									<td colspan="7" ><textarea class="form-control" style="height: 70px;" id="early_remark" name="early_remark" maxlength="300">${early.remark }</textarea></td>
								</tr>
							</tbody>
							</table>
						</div>
							...
							<tr>
								...
								<td>
									<div class="input-group width100px">
										<input type="text" class="form-control border-right-0 calDate rbo" id="out_dt" name="out_dt" dateFormat="yyyy-MM-dd" alt="출고일자" disabled="disabled"  value="${rent.out_dt }">
									</div>
								</td>
								<th class="text-right">담당자</th>
								<td>
									<input type="text" class="form-control width100px" readonly="readonly" value="${rent.out_mem_name}">
									<input type="hidden" id="out_mem_no" name="out_mem_no">
								</td>
								<th class="text-right">출고 시 비고</th>
								<td style="vertical-align: unset;">
									<input type="text" class="form-control width240px" id="out_remark" name="out_remark" maxlength="100" value="${rent.out_remark}"/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<div class="form-row inline-pd widthfix">
										<div class="col width60px">
											<input type="text" class="form-control rbo" format="decimal1" id="out_op_hour" name="out_op_hour" alt="출고 시 가동시간" value="${rent.out_op_hour }" readonly="readonly">
										</div>
										<div class="col width22px">hr</div>
									</div>
								</td>
								...
								<td>
									<div class="form-row inline-pd widthfix">
										<div class="col width60px">
											<input type="text" class="form-control rbo" format="decimal1" id="out_job_hour" name="out_job_hour" alt="출고시간" value="${rent.out_job_hour }" min="${rent.out_job_hour }" readonly="readonly">
										</div>
										<div class="col width22px">hr</div>
									</div>
								</td>
								...
								<td>
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="checkbox" id="out_gps_op_yn_check" name="out_gps_op_yn_check" disabled="disabled" value="Y" <c:if test="${rent.out_gps_op_yn == 'Y'}">checked="checked"</c:if>>
...
<%--									<input type="hidden" id="out_mem_no" name="return_mem_no">--%>
								...
								<td style="vertical-align: unset;">
									<input type="text" class="form-control width240px" id="return_remark" name="return_remark" maxlength="100" value="${rent.return_remark}"/>
								</td>
							</tr>
							<tr>
...
<%--										<c:if test="${page.fnc.F00968_001 eq 'Y'}">--%>
