...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 부품 > 발주/납기관리 > 부품발주관리 > null > 부품발주상세
-- 작성자 : 김태훈
...
<head>
	...
	<script type="text/javascript">
		...
		function fnInit() {
			modifyLevel = "${apprJobStepCd}"; // 결재중일때 승인, 심사 단계 (01 : 작성 , 02 : 심사, 03 : 승인)
			console.log(modifyLevel);
			// 결재완료면 수정불가
			if ("${bean.part_order_status_cd}" == "2") {
				modifyLevel = "09";
			}
			if ("${bean.part_order_status_cd}" == "0") {
				modifyLevel = "01";
			}
			if ("${bean.part_order_status_cd}" != "0") {
				$("#order_condition").prop("disabled", true);
				$("#special_text").prop("disabled", true);
				$("#desc_text").prop("disabled", true);
			}
			maxSeq = ${bean.max_seq_no};
			console.log(maxSeq);
			list = ${dtlList}
			console.log(list);
			warehouseCodes = ${codeMapJsonObj.WAREHOUSE}
			// $M.setValue("reg_dt", $M.dateFormat("${partOrderNo1}", 'yyyy-MM-dd'));
			if ("${bean.order_proc_dt}" != ""){
				$M.setValue("order_proc_dt", $M.dateFormat("${bean.order_proc_dt}", 'yyyy-MM-dd'));				
			}
			var total = 0;
			var total_amt = 0;
			warehouseCodes = warehouseCodes.reduce(function(map, obj) {
			    map[obj.code_value] = obj.code_name;
			    return map;
			}, {});
			for (var i = 0; i < list.length; ++i) {
				// seq 최대값 구하기
				/* var seq = parseInt(list[i].seq_no);
				if (seq > maxSeq) {
					maxSeq = seq;
				}; */
				var total_price = list[i].approval_qty*list[i].unit_price;
				var warehouseArr = list[i].warehouse_cd_array;
				var center_name_arr = [];
				if (warehouseArr != null && warehouseArr != "") {
					var temp = warehouseArr.split("^");
					for (var j = 0; j < temp.length; ++j) {
						center_name_arr.push(warehouseCodes[temp[j]]);
					}
				}
				...
				list[i]['button_text'] = buttonText;
				total+=list[i].approval_qty; // 총수량
				total_amt+=total_price;// 합계수량
				list[i]['total_price'] = total_price; // row별 합계 수량
				
				// 요청정보 가공
				// 발주요청자료에서 입력
				if (list[i].detailList != null && list[i].detailList != "") {
					var result = [];
	   				list[i].detailList.reduce(function(res, value) {
	   				...
	   				list[i]['request_order_qty'] = result[0].request_order_qty;
	   				list[i]['fixed_cnt'] = result[0].fixed_cnt;
	   				console.log("reduce result => ", result);
	   				var attrs = ['part_no', 'order_org_code'];
	   				var temp = pivotList(list[i].detailList, attrs);
	   				
	   				console.log("pivotList => ", temp);
	   				var detail = temp[0].groups;
	   				...
	   				list[i]['preorder_no_str'] = $M.getArrStr(preorder_no_arr);
				}
			}
			console.log(list);
		}
		...
		function createAUIGrid() {
			var gridPros = {
				...
				var columnLayout = [
					...
					{ 
						headerText : "계약납기일", 
						dataField : "delivary_dt", //TODO: 계약납기일
						dataType : "date",  
						width : "75",
						minWidth : "60",
						dataInputString : "yyyymmdd",
				...
				AUIGrid.bind(auiGrid, "updateRow", function( event ) {
					var total = AUIGrid.getNotDeletedColumnValues(auiGrid, "total_price");
		...
		function fnUpdateCnt() {
			...
			var removedTotalAmt = 0;
			var removedApprovalQty = 0;
			for (var i = 0; i < removedItems.length; ++i) {
				removedTotalAmt+=parseFloat(removedItems[i].total_price);
				removedApprovalQty+=parseFloat(removedItems[i].approval_qty);
			}
			var removedTotalAmt = removedTotalAmt.toFixed(2);
			var cnt = AUIGrid.getGridData(auiGrid).length - removedItems.length;
			$("#total_cnt").html(cnt);
			...
			var totalAmt = result.toFixed(2);
			totalAmt = totalAmt-removedTotalAmt;
			totalAmt = totalAmt.toFixed(2);
			if (totalAmt == "0.00") {totalAmt = "0"};
			$M.setValue("total_amt", $M.setComma(totalAmt));
			var cnt = AUIGrid.getColumnValues(auiGrid, "approval_qty");
			var cntResult = cnt.reduce(function(a, b) { return a + b; }, 0);
			$M.setValue("total", $M.setComma(cntResult-removedApprovalQty));
		}
		...
		function goBregDetailPopup() {
			...
			if (custNo == "") {
				...
				var param = {
					"cust_no" : custNo,
					"cust_name" : $M.getValue("breg_name")
				};
		...
		function goAssignPopup(event) {
			...
			var param = {
				...
				init_center_cd_str : init_center_cd_str,
				request_order_qty_str : request_order_qty_str,
				init_request_order_qty_str : init_request_order_qty_str,
		...
		function auiCellEditHandler(event) {
			...
			case "cellEditBegin" :
				...
				// 작성중이 아니면 아래 항목 수정 불가
				if (event.dataField == "part_no" || 
					event.dataField == "in_plan_dt" || // 입고예정
					event.dataField == "unit_price" || // 단가
					event.dataField == "remark" || // 적요
					event.dataField == "delivary_dt"
					) {
						if (modifyLevel != "01") {
							return false;
						}
				...
				if (event.dataField == "check_qty") {
					console.log(modifyLevel);
					if (modifyLevel != "02") {
						return false;
					}
				}
				if (event.dataField == "approval_qty") {
					console.log(modifyLevel);
					if (modifyLevel != "03") {
						return false;
					}
			...
			case "cellEditEndBefore" :
				if(event.dataField == "order_qty" || event.dataField == "check_qty" || event.dataField == "approval_qty") {
					console.log(event);
					if(event.item.preorder_inout_doc_no_str != 0) {
						alert("선주문수량이 포함되어있으므로, 수정 시 참고하세요.");
					}
					if (event.value < event.item.fixed_cnt){
						// 요청수량 밑으로 예외처리 삭제(이원영파트장 지시)
						/* setTimeout(function() {
			...
			case "cellEditEnd" :
				if(event.dataField == "part_no") {
					...
					fnCalcTotal(event.dataField);
				} else if (event.dataField == "check_qty") {
					AUIGrid.updateRow(auiGrid, {
						approval_qty : event.value,
						total_price : event.value * event.item.unit_price
					}, event.rowIndex);
					fnCalcTotal(event.dataField);
				} else if (event.dataField == "approval_qty") {
					AUIGrid.updateRow(auiGrid, {
						total_price : event.value * event.item.unit_price
					}, event.rowIndex);
					fnCalcTotal(event.dataField);
		...
		function fnCalcTotal(dataField) {
			var orderQty = AUIGrid.getNotDeletedColumnValues(auiGrid, dataField);
		...
		// 수정 및 결재반영
		function goUpdate(isRequestAppr) {
			if (AUIGrid.getGridData(auiGrid).length == 0) {
				alert("부품을 추가하세요.");
				return false;
			}
			
			var isAllRemoved = AUIGrid.getGridData(auiGrid);
			var removeCnt = 0;
			for (var i = 0; i < isAllRemoved.length; ++i) {
				if (AUIGrid.isRemovedById(auiGrid, isAllRemoved[i]._$uid) == true) {
					removeCnt+=1;
				}
			}
			if (removeCnt == isAllRemoved.length) {
				alert("발주할 부품이 없습니다.");
				return false;
			}
			...
			if (isRequestAppr != undefined) {
				...
				if (isRequestAppr == "approval") {
					$M.setValue("save_mode", "update_approval"); // 승인
				} else {
					// 결재요청
					$M.setValue("save_mode", "update_appr"); // 결재요청
					if(confirm("결재 후 수정 및 삭제가 제한됩니다.\n계속 진행하시겠습니까?") == false) {
						return false;
					}					
				}
			} else {
				$M.setValue("save_mode", "update"); //저장
			...
			// 결재처리 후 필요한 항목만 서버로 보내기위해 가공
			if (isRequestAppr == "approval") {
				/* var approvalTemp = AUIGrid.getEditedRowItems(auiGrid);
				// 그리드에서 결재처리할때 필요한 값만 추출함
				var result = approvalTemp.map(function(item) {
	  				return {
	  					  	seq_no : item["seq_no"]
	  					  , part_no : item["part_no"]
	  					  , check_qty : item["check_qty"]
	  					  , approval_qty : item["approval_qty"]
	  					  , cmd : 'U'
	  				}
				});
				var form = $M.jsonArrayToForm(result); */
				...
				$M.goNextPageAjax("/part/part0403p01/appr", gridForm, {method : 'POST'},
						function(result) {
				    		if(result.success) {
				    			alert("처리가 완료되었습니다.");
								location.reload();
				...
				processAssgin(tempGrid);
				var tempModifyGrid = AUIGrid.getEditedRowItems(auiGrid);
				processAssgin(tempModifyGrid);
				frm = $M.toValueForm(frm);
				
				// 모든 로우를 강제로 수정상태로 바꿈
				var forceUpdatedGrid = AUIGrid.getGridData(auiGrid);
				for (var i = 0; i < forceUpdatedGrid.length; ++i) {
					if (AUIGrid.isRemovedById(auiGrid, forceUpdatedGrid[i]._$uid) == false) {
						AUIGrid.updateRow(auiGrid, {"garbage":1}, AUIGrid.rowIdToIndex(auiGrid, forceUpdatedGrid[i]._$uid));
						AUIGrid.updateRow(auiGrid, {"use_yn":"Y"}, AUIGrid.rowIdToIndex(auiGrid, forceUpdatedGrid[i]._$uid));
					} else {
						AUIGrid.updateRow(auiGrid, {"use_yn":"N"}, AUIGrid.rowIdToIndex(auiGrid, forceUpdatedGrid[i]._$uid));
					}
				}
				...
				$M.copyForm(gridForm, frm);
				console.log("save ==> ", gridForm);
				goSavePartOrder(gridForm);
			}
		}
		
		function goSavePartOrder(obj) {
			/* alert("오류 수정중입니다.");
			return false; */
			$M.goNextPageAjax("/part/part040301/save", obj, {method : 'POST'},
					function(result) {
			    		if(result.success) {
			    			alert("처리가 완료되었습니다.");
							location.reload();
		...
		// 센터할당 처리
		function processAssgin(tempGrid) {
		...
		function goRequestApproval() {
			goUpdate('requestAppr');
		}
		
		// 매입처조회 팝업
		function fnSearchClientComm() {
			if ("${bean.part_order_status_cd}" != "0") {
				alert("작성중인 자료만 가능합니다.");
				return false;
			}
			var param = {};
		...
		function goPrintOrder(Language) {
			if(Language == "KO"){
				openReportPanel('part/part0403p01_01.crf','part_order_no=' + $M.getValue("part_order_no"));
			}else{
				openReportPanel('part/part0403p01_02.crf','part_order_no=' + $M.getValue("part_order_no"));
			}
		}
		...
		function goApprovalResult(result) {
			console.log(result);
			// 반려이면 페이지 리로딩
			//if(result.appr_proc_status_cd == '03') {
		...
		// 발주
		function goOrderProcess() {
			if (confirm("발주처리하시겠습니까?") == false) {
				return false;
			}
			$M.goNextPageAjax(this_page+"/${bean.part_order_no}/order", '', {method : 'POST'},
				function(result) {
		    		if(result.success) {
		    			location.reload();
		    			//opener.goSearch();
		...
		function goRemove() {
			...
			if (confirm("작성중인 발주자료를 삭제하시겠습니까?") == false) {
				console.log("d");
				return false;
			}
			$M.goNextPageAjax(this_page+"/${bean.part_order_no}/remove", '', {method : 'POST'},
				function(result) {
		    		if(result.success) {
		    			alert("삭제에 성공했습니다.");
		    			fnClose();
        ...
        <div class="content-wrap">	
			<div class="title-wrap half-print" style="min-width: 1000px;">
				<div class="doc-info" style="flex: 1;">				
					<h4 class="primary">부품발주상세</h4>
					<div>
						<span class="condition-item">상태 : ${bean.part_order_status_name}</span>
						<!-- 발주 또는 마감일때 발주서 한글 영문 인쇄(3 : 발주, 9 : 마감) -->
						<c:if test="${bean.part_order_status_cd == 3 || bean.part_order_status_cd == 9}">
							<button type="button" class="btn btn-md btn-rounded btn-outline-primary" onclick="goPrintOrder('KO')"><i class="material-iconsprint text-primary"></i> 발주서 한글인쇄</button>		
							<button type="button" class="btn btn-md btn-rounded btn-outline-primary" onclick="goPrintOrder('EN')"><i class="material-iconsprint text-primary"></i> 발주서 영문인쇄</button>
						</c:if>
					</div>			
				</div>
<!-- 결재영역 -->
...
<!-- 폼테이블 -->
			<div class="row mt5" style="min-width: 1000px">
<!-- 좌측 폼테이블-->
						...
						<table class="table-border">
							...
							<tbody>
									...
									<td>
										<div class="form-row inline-pd widthfix">
											<div class="col width120px" style="padding-right: 0;">
												<input type="text" class="form-control" readonly="readonly" value="${bean.dis_part_order_no}"> <!-- 페이지 접속할때의 현재 날짜 -->
											</div>
											<%-- <div class="col-1" style="padding-right: 0;text-align: center;">-</div>
											<div class="col width45px">
												<input type="text" class="form-control" readonly="readonly" value="${bean.order_no}"> <!-- 페이지 들어올때마다 증가에서 저장 후 채번으로 변경하기로함 -->
											</div> --%>
										</div>
									</td>
									<th class="text-right">담당자</th>
									<td>
										<input type="text" class="form-control width120px" readonly="readonly" value="${bean.reg_mem_name}">
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="input-group">
											<input type="text" class="form-control border-right-0 width120px" readonly="readonly" id="cust_name" name="cust_name" required="required" alt="발주처" value="${bean.cust_name}">
									...
									<td>
										...
										<input type="text" class="form-control width140px" readonly="readonly" id="cust_hp_no" name="cust_hp_no" alt="휴대폰" value="${bean.cust_hp_no}">
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="input-group width120px">
											<input type="text" class="form-control border-right-0" id="current" name="current" disabled="disabled" value="${bean.reg_dt}" dateformat="yyyy-MM-dd">
									...
									<td>
										<input type="text" class="form-control width120px" readonly="readonly" alt="발주처리일" id="order_proc_dt" name="order_proc_dt" value="${bean.order_proc_dt}">
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="form-row inline-pd widthfix">
											<div class="col width120px">
												<input type="text" class="form-control text-right width120px" readonly="readonly" id="total_amt" name="total_amt" value="0" alt="합계금액" required="required" value="${bean.total_amt}" format="decimal">
											</div>
								...
								<tr>
									...
									<td colspan="3">
										<textarea class="form-control" style="height: 50px; resize: none;" id="order_condition" name="order_condition" alt="발주조건(외부용)" maxlength="4000">${bean.order_condition}</textarea>
									</td>
								</tr>
								<tr>
									<th class="text-right">특이사항(내부용)</th>
									<td colspan="3">
										<textarea class="form-control" style="height: 50px; resize: none;" id="special_text" name="special_text" alt="특이사항(내부용)" maxlength="4000">${bean.special_text}</textarea>
									</td>
								</tr>
							</tbody>
						</table>
							...
							<tbody>
								<tr>
									...
									<td colspan="3">
										<div class="form-row inline-pd widthfix">
											<div class="col width120px">
												<input type="text" class="form-control text-left width120px" readonly="readonly" id="breg_name" name="breg_name" alt="업체명" value="${bean.breg_name}">
											</div>
								...
								<tr>
									...
									<td>
										<input type="text" class="form-control width120px" readonly="readonly" id="breg_rep_name" name="breg_rep_name" alt="대표자" value="${bean.breg_rep_name}">
									</td>
									<th class="text-right">전화번호</th>
									<td>
										<input type="text" class="form-control width140px" readonly="readonly" id="cust_tel_no" name="cust_tel_no" alt="전화번호" value="${bean.cust_tel_no}">
									</td>
								</tr>
								<tr>
									<th class="text-right">사업자No</th>
									<td>
										<input type="text" class="form-control width140px" readonly="readonly" id="breg_no" name="breg_no" alt="사업자번호" value="${bean.breg_no}">
									</td>
									<th class="text-right">팩스번호</th>
									<td>
										<input type="text" class="form-control width140px" readonly="readonly" id="cust_fax_no" name="cust_fax_no" alt="팩스번호" value="${bean.cust_fax_no}">
									</td>
								</tr>
								<tr>
									<th class="text-right">적요</th>
									<td colspan="3">
										<input type="text" class="form-control" id="desc_text" name="desc_text" maxlength="200" alt="적요" value="${bean.desc_text}">
									</td>
								</tr>
								<tr>
...
<!-- 부품내역 -->
				<div class="title-wrap mt10">
					...
					<div>
						<!-- 상태에 따라 변경때문에 공통안씀 -->
						<c:if test="${bean.part_order_status_cd == '0'}">
							<button type="button" class="btn btn-default" onclick="javascript:fnMassInputPart()"><i class="material-iconsbuild text-default"></i>부품대량입력</button>
							<button type="button" class="btn btn-default" onclick="javascript:goPartList()"><i class="material-iconsbuild text-default"></i>부품조회</button>
							<button type="button" class="btn btn-default" onclick="javascript:goOrderReferPopup()">발주요청자료</button>
							<button type="button" class="btn btn-default" onclick="javascript:fnAdd()"><i class="material-iconsadd text-default"></i>10행추가</button>						
						</c:if>
...
<input type="hidden" id="appr_job_seq" name="appr_job_seq" value="${bean.appr_job_seq}">
<input type="hidden" id="part_order_no" name="part_order_no" value="${bean.part_order_no}" >
<input type="hidden" id="cust_no" name="cust_no" required="required" value="${bean.cust_no}">
<input type="hidden" id="breg_seq" name="breg_seq" value="${bean.breg_seq}">
<input type="hidden" id="part_order_status_cd" name="part_order_status_cd" value="${bean.part_order_status_cd}">
<input type="hidden" id="save_mode" name="save_mode">
<input type="hidden" id="machine_ship_plan_seq" name="machine_ship_plan_seq" value="${bean.machine_ship_plan_seq}">
</form>
</body>
</html>