		...
		for (var i = 0; i < gridAllList.length; i++) {			
		   for (var j = 0; j < checkedRowItems.length; j++) {	
			   ...
			   if (gridAllList[i].part_no == checkedRowItems[j].part_no){
					// HOMI이동 요청수량이 1보다 작은경우 
					if( checkedRowItems[j].qty <= 0) {					

						alert("체크된 정보의 HOMI이동 요청수량을 입력해주세요.");		
						AUIGrid.showToastMessage(auiGrid, i, 9, "체크된 정보의 HOMI이동 요청수량을 입력해주세요.");
						return;
					}			   
			   }			  				
		    }		
								...
								<tr>
									...
									<td>
										<div class="form-check form-check-inline">
											<input class="form-check-input multi-check" type="checkbox" name="s_homi_yn" id="s_homi_yn"  value="Y" checked="checked" onclick="javascript:fn_validCheck(this);">
											<label for="s_homi_yn" class="form-check-label">HOMI 지정품</label>
										</div>
										<div class="form-check form-check-inline">
											<input class="form-check-input multi-check" type="checkbox" name="s_current_under_safe" id="s_current_under_safe" value="Y" onclick="javascript:fn_validCheck(this);">
											<label for="s_current_under_safe" class="form-check-label">센터재고 &lt; 적정재고</label>
										</div>
										<div class="form-check form-check-inline">
											<input class="form-check-input multi-check" type="checkbox" name="s_out_mng_yn" id="s_out_mng_yn" value="Y" onclick="javascript:fn_validCheck(this);">
