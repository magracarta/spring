...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 회계 > 인사 > 발령관리 > 발령 등록 > null
-- 작성자 : 이강원
-- 최초 작성일 : 2021-05-28 14:09:45
...
// 										item.new_menu_org_code = menuOrgList[i].org_code;
// 										return menuOrgList[i].org_name;
// 									}
// 								}
// 								return "";
...
// 					},
			...
			{ 
				...
				renderer : {
					...
					onClick : function(event) {
						fnRemoveAuiData(event.rowIndex);
					},
				},
				labelFunction : function(rowIndex, columnIndex, value,headerText, item) {
					return '삭제'
		...
		AUIGrid.bind(auiGrid,"cellEditBegin",function(event){
			if(event.dataField == "new_org_code"){
		...
		AUIGrid.bind(auiGrid,"cellEditEnd",function(event){
			...
			if(event.dataField == "mem_move_cd"){
				...
				if(event.item.mem_move_cd == '04' || event.item.mem_move_cd == '90'){
	...
	// 직원삭제
	function fnRemoveAuiData(row){
		AUIGrid.removeRow(auiGrid,row);
	}
	...
	function goCheckPayment(memNo, row , reCheck){
		...
		if(check == 'Y'){
			...
			$M.goNextPageAjax(this_page + "/check", $M.toGetParam(param) , {method : 'GET'},
					function(result) {
			    		if(result.success) {
	...
	function goSave(){
		...
		$.each(data,function(rowIndex){
			...
			if(data[rowIndex].new_job_cd=="" || data[rowIndex].new_job_cd==undefined) {
	...
	<!-- /메인 타이틀 -->	
			...
			<!-- 기본 -->					
					...
					<table class="table-border mt5">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" id="mem_move_no" name="mem_move_no" value="${memMoveNo}" readonly="readonly"/></td>
							</tr>								
						</tbody>
					</table>
				</div>
				...
				<div class="title-wrap mt10">
					...
					<div class="btn-group">
						<div class="right">
							<button type="button" class="btn btn-default" onclick="javascript:openMemberOrgPanel('setMemberOrgMapPanel', 'Y');"><i class="material-iconsadd text-default"></i> 직원추가</button>
						</div>
					</div>
				</div>
	<!-- /그리드 타이틀, 컨트롤 영역 -->					
