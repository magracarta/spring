...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 회계 > 정산 > 출하시임의비용처리 > 원가 반영 > null
-- 작성자 : 정재호
...
<head>
	...
	<script type="text/javascript">
		...
		var costApplyJson = JSON.parse('${codeMapJsonObj['COST_APPLY']}');

		$(document).ready(function() {
			createAUIGrid();
			// Q&A 18349 와 같이 iframe 로딩 시 해당 탭이 아닐지라도 검색결과가 나오는 경우가 있어서
			// 페이지 로딩시에는 조회 안하도록 수정
			// goSearch();
			fnInit();
		});
...
// 				},
				{
					...
					labelFunction : function(  rowIndex, columnIndex, value, headerText, item ) {
						var retStr = value;
						for(var j = 0; j < costApplyJson.length; j++) {
							if(costApplyJson[j]["code_value"] == value) {
								retStr = costApplyJson[j]["code_name"];
								break;
							}
						}
						return retStr;
		...
		function fnDownloadExcel() {
			...
			fnExportExcel(auiGrid, "출하시임의비용처리_원가반영", exportProps);
		}
		...
		function goCostProcess() {
			...
			var cmdArr = [];
			var costProcYnArr = [];
			var costApplyCdArr = []; // 원가반영 code_apply_name 값 리스트
			var costItemCdArr = []; // 원가반영 code_item_cd 정보 리스트 (99로 고정)
			var costNameArr = [];
			var amtArr = [];
			for (var i = 0; i < rows.length; ++i) {
				machineDocNoArr.push(rows[i].machine_doc_no);
				seqNoArr.push(rows[i].seq_no);
				costApplyCdArr.push(rows[i].cost_apply_cd);
				costItemCdArr.push('99');
			...
			var param = {
				...
				cost_apply_cd_str : $M.getArrStr(costApplyCdArr),
...
//  					amt_str : $M.getArrStr(amtArr),
		...
		function fnSetOppCost(data) {
			var colIndex = AUIGrid.getColumnIndexByDataField(auiGrid, "cost_apply_cd");
			fnSetCellFocus(auiGrid, colIndex, "cost_apply_cd");
			var row = new Object();
		...
		function isValid() {
			...
			return AUIGrid.validateGridData(auiGrid, ["machine_doc_no", "cost_apply_cd", "cost_name", "amt"], "필수 항목는 반드시 값을 입력해야 합니다.");
		}
		...
		function goSave() {
			...
			var machineDocNoArr = [];
			var costApplyCdArr = []; // 원가반영 실제 구분 정보 리스트 (cost_apply_name) 값
			var costNameArr = [];
			var amtArr = [];
			var costProcYnArr = [];
			var cmdArr = [];
			...
			var param = {
				...
				cost_apply_cd_str : $M.getArrStr(costApplyCdArr, option),
