...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 서비스 > 서비스관리 > 서비스업무평가-센터 > 실적분석 > 유상_무상 정비건수 변화추이
-- 작성자 : 성현우
...
<head>
	...
	<script type="text/javascript">
		...
		var asRepairTotalCountJson = ${listJson}; // 유상+무상 정비건수 데이터
		...
		function fnCreateChart() {
			// 데이터
			var dataList = asRepairTotalCountJson;
				...
				for(var i=0; i<itemLength; ++i) {
					...
					for(var prop in row) {
						if(prop == "yyyymm") {
							...
							dataArray[index] += $M.toNum(row["as_repair_tot"]);
						}
					}
				}
