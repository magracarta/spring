...
<head>
	...
	function fnInit() {
		var cowokerList = [];
		<c:if test ="${not empty cowokerList}">
		 cowokerList = ${cowokerList}
		</c:if>
		
		if ('R' == '${inputParam.read_type}') {
			$("#_goApply").addClass("dpn");
		}
		
		console.log(cowokerList.length);
		
		if('${inputParam.s_type}' != 'D' && cowokerList.length < 1) {
			var item = new Object;
			item.mem_no = '${SecureUser.mem_no}';
			item.mem_name = '${SecureUser.user_name}';
			item.work_rate = "";
			item.work_amt = "";
	...
	function setMemberOrgMapPanel(data) {
		...
		for(var i in data) {
			if (!isGridData(auiGrid, "mem_no", data[i].mem_no)) {
				var item = new Object();
				item.mem_name = data[i].mem_name;
				item.mem_no = data[i].mem_no;
				item.work_rate = "";
				item.work_amt = "";
				item.use_yn = "Y";
	...
	function goApply() {
		...
		for(var i=0; i<data.length; i++) {
			workRate += $M.toNum(data[i].work_rate);
			workAmt += $M.toNum(data[i].work_amt);
		}

		if(data.length > 0 && workRate != 100) {
			alert("공임비율의 총 합을 100%로 만들어 주세요.");
			return;
		}
		
		// 채상무님 9.28일 회의 지시사항
		if(data.length > 0 && workAmt == 0) {
	...
	function createAUIGrid() {
		...
		AUIGrid.setGridData(auiGrid, ${cowokerList});
	...
	function auiCellEditHandler(event) {
		switch (event.type) {
			case "cellEditEnd" :
				if(event.dataField == "work_rate") {
					var workTotalAmt = '${inputParam.work_total_amt}';
					workTotalAmt = $M.toNum(workTotalAmt);
					var workRate = $M.toNum(event.item.work_rate);
					var result = workTotalAmt * workRate / 100;

					AUIGrid.updateRow(auiGrid, {"work_amt" : result}, event.rowIndex);
				}
				break;
		}
	}
		...
		<div class="content-wrap">
			...
			<div class="title-wrap mt5">
				...
				<div class="right half-print">
					<span class="condition-item mr5">정비공임 : ${inputParam.work_total_amt} 원</span>
