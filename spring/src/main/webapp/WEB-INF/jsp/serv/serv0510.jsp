...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 서비스 > 서비스관리 > 전화상담분석 > null > null
-- 작성자 : 성현우
			...
			$M.goNextPageAjax(this_page + "/search", $M.toGetParam(params), {method: 'GET'},
					function (result) {
						if (result.success) {
							...
							// console.log(AUIGrid.getGridData(auiGrid));
						}
					}
			);
		}
		...
		function createAUIGrid() {
			var gridPros = {
				...
				rowStyleFunction : function(rowIndex, item) {
					...
					// }
					//
					// if(rowIndex == 12 || rowIndex == 0 || rowIndex == 1) {
					// 	return "aui-as-tot-row-style"
					// }
				}
			};
			...
			for (var i = 0; i < centerListJson.length; i++) {
				...
				var centerRateObj = {
					...
					labelFunction: function (rowIndex, columnIndex, value, headerText, item) {
						...
						if (value == 0) {
							str = "";
						} else if (!Number.isInteger(value) && item.center_01.indexOf("1인당") == -1) {
							str = value.toFixed(1) + "%";
						} else {
			...
			AUIGrid.bind(auiGrid, "cellClick", function( event ) {
				...
				// 총 장비대수 셀 && 전화상담장비대수 1개 셀
				if(event.item.type_code == 'tot_machine_cnt' || (event.item.type_code == 'call_machine_cnt') && (event.columnIndex) % 2 == 0) {
					...
					$M.goNextPage('/serv/serv0510p01', $M.toGetParam(params), {popupStatus : ''});
				}

				// 총 전화상담건수
				if(event.item.type_code == 'tot_call_cnt') {
					var centerOrgCode = event.dataField.split('_')[1]; // 센터 코드
					var params = {
						"s_center_org_code" : centerOrgCode,
						"s_start_dt" : $M.getValue("s_start_dt"),
					...
					$M.goNextPage('/serv/serv0510p02', $M.toGetParam(params), {popupStatus : ''});
				}

				// if((event.item.type_code == 'tot_machine_cnt') // 총 장비대수
				// 	|| (event.item.type_code == 'tot_call_cnt')  // 총 전화상담건수
				// 	|| ((event.item.type_code == 'call_machine_cnt') && (event.columnIndex) % 2 == 0) // 전화상담장비대수의 갯수 셀
				// ){
				// 	var centerOrgCode = event.dataField.split('_')[1];
				// 	var params = {
				// 		"s_center_org_code" : centerOrgCode,
				// 		"s_cnt" :
				// 		"s_start_dt" : $M.getValue("s_start_dt"),
							...
							<tr>
								...
								<td>
									<div class="left text-warning">
										[※ 목표전화상담건수 : 총 장비대수 * 조회기간(개월 수) / 12 * 3 || 총 전화상담 접촉률 : 0건을 제외한 전화상담장비대수 건수 및 비율 합계]
									</div>
								</td>
							</tr>
							</tbody>
