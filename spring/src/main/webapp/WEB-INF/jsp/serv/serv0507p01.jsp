...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 서비스 > 서비스관리 > SA-R신청관리 > null > SA-R계약정보
-- 작성자 : 최보성
-- 최초 작성일 : 2020-04-08 13:29:29
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	function fnInitPage() {
		...
		} else if(sarStatusCd == "C") {
			$("#_goCompanion").addClass("dpn");
		}

		var fileSeq = '${map.file_seq}';
		var fileName = '${map.file_name}';
		if(fileSeq != "0") {
			var file = {
				file_seq : fileSeq,
				file_name : fileName
...
// 	}
	...
	function goChangeSave() {
		...
		$M.goNextPageAjaxSave(this_page+"/saveMemo", $M.toGetParam(param), {method : "POST"},
				function(result) {
					if(result.success) {
						location.reload();
					};
							...
							<table class="table-border mt5">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<input type="text" id="contract_no" name="contract_no" value="${map.contract_no}" class="form-control width160px statusC-input"  ${map.machine_sar_status_cd eq 'R' ? '' : 'readonly'}>
										</td>
									</tr>
									<tr>
