...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 서비스 > 서비스관리 > 서비스업무평가-센터 > 실적분석 > 센터별 실적분석
-- 작성자 : 손광진
-- 최초 작성일 : 2020-04-08 13:02:29
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	<script type="text/javascript">
		...
		function createAUIGridA() {
			...
			footerColumnLayout[1] = [
				...
				},
				{
					...
					expFunction : function(columnValues) {
						...
						for (var i = 0; i < gridData.length; i++) {
							sum += $M.toNum(gridData[i].free_part_amt);
						}
				...
				},
				{
					...
					expFunction : function(columnValues) {
						...
						for (var i = 0; i < gridData.length; i++) {
							sum += $M.toNum(gridData[i].mbo_amt);
						}
			...
			footerColumnLayout[2] = [
				...
				},
				{
					...
					expFunction : function(columnValues) {
						...
						for (var i = 0; i < gridDataA.length; i++) {
							sumA += $M.toNum(gridDataA[i].free_part_amt);
						}
						for(var i=0; i<gridDataB.length; i++) {
							sumB += $M.toNum(gridDataB[i].free_part_amt);
						}
				...
				},
				{
					...
					expFunction : function(columnValues) {
						...
						for (var i = 0; i < gridDataA.length; i++) {
							sumA += $M.toNum(gridDataA[i].mbo_amt);
						}
						for(var i=0; i<gridDataB.length; i++) {
							sumB += $M.toNum(gridDataB[i].mbo_amt);
						}
			...
			AUIGrid.bind(auiGrid, "cellClick", function (event) {
				...
				// 무상정비금액
				if(event.dataField == "free_part_amt" || event.dataField == "free_travel_amt" || event.dataField == "free_work_amt") {

					/* if (event.dataField == "free_part_amt") {
						params.type = "1"; // 부품비
					...
					if (event.dataField == "free_travel_amt" || event.dataField == "free_work_amt" || event.dataField == "free_part_amt") {
						params.type = "2"; // 출장비,공임
					}
