...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 모바일관리 > > 메뉴내기능 > null > null
-- 작성자 : 이강원
-- 최초 작성일 : 2023-01-30 17:05:48
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
				...
				function(result) {
					if(result.success){
						...
						if(menuChk != undefined) {
							$M.setValue("menu_fnc_no", "F" + $M.lpad($M.getValue("m_menu_seq"), 5, 0) + "_" + $M.lpad(maxSeq, 3, "0"));
						} else {
							fnNew(menuSeq);
		...
		$M.goNextPageAjax(this_page + "/detail/fnc", $M.toGetParam(param),{ method : 'get'},
				function(result) {
					if(result.success){
						...
						$M.setValue("menu_name", result.menuFnc.m_menu_seq);
	...
	function goSave() {
		...
		$M.goNextPageAjaxSave(this_page + "/save", frm, { method : 'POST'},
				function(result) {
					if(result.success) {
						goSearchDetail($M.getValue("m_menu_seq"));
					}
				}
		);
	}
	...
	function fnNew(menuSeq) {
		...
		$M.setValue("m_menu_seq", menuSeq != undefined ? menuSeq : $M.getValue("m_menu_seq"));
		$M.setValue("menu_name", menuSeq != undefined ? menuSeq : $M.getValue("m_menu_seq"));
		$M.setValue("cmd", "C");
		var param = {
			m_menu_seq : $M.getValue("m_menu_seq"),
			menu_fnc_no : $M.getValue("m_menu_seq") == "" ? "" : "F" + $M.lpad($M.getValue("m_menu_seq"), 5, 0) + "_" + $M.lpad(maxSeq, 3, "0"),
			menu_fnc_name : "",
			mem_no_str : "",
			mem_no_op : "OR",
			mem_no_yn  : "",
	...
	function createAUIGrid() {
		...
		AUIGrid.bind(auiGrid, "cellClick", function(event){
			if (event.headerText == "메뉴명") {
				goSearchDetail(event.item["m_menu_seq"]);
			}
		});
   }
	...
	function createAUIGridFnc() {
		...
		AUIGrid.bind(auiGridFnc, "cellClick", function(event){
			if (event.headerText == "기능번호") {
				$M.setValue("cmd", "U");
				goSearchFncDetail(event.item.m_menu_seq, event.item.menu_fnc_no);
			}
		});
	}
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="btn-group">
															<div class="left">
																<input type="text" class="form-control width230px" id="m_menu_seq" name="m_menu_seq" alt="메뉴번호" readonly>
															</div>
															<div class="right">
																<div class="form-check form-check-inline checkline">
													...
													<td>
														<select id="menu_name" name="menu_name" class="form-control essential-bg width280px" style="height:24px; max-width: 280px;" onchange="fnChangeMenu()">
															...
															<c:forEach items="${menuList}" var="item">
																<option value="${item.m_menu_seq}">${item.path_menu_name}</option>
															</c:forEach>
														</select>
													</td>
												</tr>
