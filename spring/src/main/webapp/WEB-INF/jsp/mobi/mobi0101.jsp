...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 모바일관리 > 기준정보 > 메뉴관리 > null > null
-- 작성자 : 황빛찬
-- 최초 작성일 : 2023-02-01 14:37:22
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	var btnAccessArray = [{code_value : "G", code_name : "일반"}, {code_value : "W", code_name : "작성자"}];
   ...
   //상위메뉴 select
	function goUpMenuSearch(up_m_menu_seq) {
	   console.log(up_m_menu_seq);
	   var param = {}
		$M.goNextPageAjax(this_page + "/upmenu", $M.toGetParam(param) , { method : 'get'},
			function(result) {            
				if(result.success){
					...
					$("#up_m_menu_seq").append("<option value=''>- 선택 -</option>");
					for(var i = 0 ; i < comboList.length ; i++) {
						var option = $("<option></option>");
						option.val(comboList[i].m_menu_seq);
						option.text(comboList[i].path_menu_name);	
						$("#up_m_menu_seq").append(option);
					};
					if (up_m_menu_seq != undefined){
						$M.setValue("up_m_menu_seq",up_m_menu_seq);
					}
					// $M.setValue("up_m_menu_seq","");
				};
			}
		)
	}
	...
	function goSave() {
		...
		//대메뉴인경우 상위메뉴번호 0셋팅
		if(bigMenuChk) {
			$("#up_m_menu_seq").attr("disabled",false);   
			$("#up_m_menu_seq").append("<option value='0'></option>");
			$M.setValue("up_m_menu_seq","0");
		};

		if($M.getValue("up_m_menu_seq") == '' && !bigMenuChk) {
			alert("상위메뉴는 필수입력입니다.");
			return;
		};
			...
			$M.goNextPageAjaxSave(this_page, frm , { method : 'POST'},
				function(result) {
					if(result.success) {
						...
						// fnNew();
						goSearch();
						goUpMenuSearch($M.getValue("up_m_menu_seq"));
					}
				}
			)
		// cmd가 C가 아니면 수정
	...
	function goUpdate() {
		...
		$M.goNextPageAjax(this_page + "/" + $M.getValue("m_menu_seq"), frm , { method : 'POST'},
			function(result) {
				if(result.success) {
					AUIGrid.setGridData(auiGrid, result.list);		
					// fnNew();
	...
	function goUpdateMenuBtn(data) {
		$M.goNextPageAjaxSave(this_page + "/"+data[0].m_menu_seq+"/btn/modify", document.data_form , { method : 'POST'},
			function(result) {
				if(result.success){
					goSearchMenuDetail();
				}
	...
	function goDeleteMenuBtn(data) {
		...
		$M.goNextPageAjax(this_page + "/"+data[0].m_menu_seq+"/btn/remove", document.data_form , { method : 'POST'},
			function(result) {
				if(result.success){
					goSearchMenuDetail();
				}
	...
	function goInsertMenuBtn(data) {
		...
		$M.goNextPageAjax(this_page + "/"+data[0].m_menu_seq+"/btn", document.data_form , { method : 'POST'},
			function(result) {
				if(result.success) {
					goSearchMenuDetail();
				}
	...
	function goSearchMenuDetail() {
		...
		$M.goNextPageAjax(this_page + "/"+param.m_menu_seq+"/detail", $M.toGetParam(param),{ method : 'get'},
			function(result) {
				if(result.success){
	...
	function fnSetValuesForHiddenDiv(data, method) {
		...
		for(var key in data) {
			...
			subRows.m_menu_seq = $M.getValue("m_menu_seq");
   ...
   function goSearchDetail(param) {
		...
		console.log(param);
		$M.goNextPageAjax(this_page + "/" + param.m_menu_seq, '', { method : 'get'},
			function(result) {
				if(result.success) {          
	...
	function fnUpMenuAction() {
		...
		if(upMenuChk) {
			$("#up_m_menu_seq").attr("disabled",true);
			$("#up_m_menu_seq").attr("class","readonly"); 
		} else {
			$("#up_m_menu_seq").attr("disabled",false);
			$("#up_m_menu_seq").attr("class",""); 
		};
	...
	function createBtnToNonPageAUIGrid() {
		...
		AUIGrid.bind(auiGridBtnLeft, "rowAllChkClick", function( event ) {
			if(event.checked) {
				var uniqueValues = AUIGrid.getColumnDistinctValues(event.pid, "m_btn_seq");
				AUIGrid.setCheckedRowsByValue(event.pid, "m_btn_seq", uniqueValues);
			} else {
				AUIGrid.setCheckedRowsByValue(event.pid, "m_btn_seq", []);
			}
		});
		// 그리드 갱신
	...
	function goSaveDetail() {
		...
		$M.goNextPageAjaxSave(this_page + "/"+$M.getValue('m_menu_seq')+"/add", gridFrm , { method : 'POST'},
			function(result) {
				if(result.success) {
					goSearchMenuDetail();
				}
	...
	function fnAdd() {
		if(fnCheckGridEmpty()) {
			var item = new Object();
			item.m_menu_seq = $M.getValue('m_menu_seq');
			item.accept_url = "";
	...
	function goSaveUrls() {
		...
		$M.goNextPageAjaxSave(this_page + "/"+$M.getValue('m_menu_seq')+"/url", gridFrm , { method : 'POST'},
				function(result) {
					if(result.success) {
						goSearchMenuDetail();
					}
											...
											<tbody>
												...
												<tr>
													...
													<td>
														...
														<input type="text" id="m_menu_seq" name="m_menu_seq" disabled="disabled" style="position: absolute;">
													</td>
													<th class="text-right essential-item">상위메뉴</th>
													<td colspan="3">
														<div class="form-row inline-pd">
															<div class="col-auto">
																<select id="up_m_menu_seq" name="up_m_menu_seq" class="form-control essential-bg width280px" style="height:24px; max-width: 280px;">
