...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 모바일관리 > 기준정보 > 부서권한관리 > null > 메뉴일괄적용
-- 작성자 : 정선경
-- 최초 작성일 : 2023-01-31 11:12:41
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	<script type="text/javascript">
		...
		function goSave() {
			...
			$M.goNextPageAjaxSave(this_page + "/save/menu", frm, {method : 'POST'},
				function(result) {
			    	if(result.success) {
						alert("저장이 완료되었습니다.");
		...
		function goSaveDetail() {
			...
			var mMenuAddCds = getItemsByField(auiGridRightBottom, "add_chk", "m_menu_add_cd");
			var addCheck = "";
			...
			$M.goNextPageAjaxSave(this_page + "/save/dtl", $M.toGetParam(param), {method : "POST"},
					function(result) {
						if(result.success) {
							alert("저장이 완료되었습니다.");
							$M.setValue("save_org_code_str",$M.getValue("org_code_str"));
		...
		function goSearchDetail(getParam) {
			...
			$M.goNextPageAjax(this_page + "/" + getParam.m_menu_seq, $M.toGetParam(param), {method : 'get'},
				function(result) {
					if(result.success) {
						// 데이터 그리드 세팅
...
<%--								<td>--%>
<%--									<select class="form-control" id="search_auth_cd" name="search_auth_cd" style="width: 100%;" disabled="disabled">--%>
