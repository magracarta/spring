...
<jsp:include page="/WEB-INF/jsp/common/auiHeader.jsp"/>
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			// 대차상담일경우 세팅
			var replaceYn = ${replace_cust_consult};
			if (replaceYn == "") {
...
// 						$M.goNextPage('/cust/cust0101p01', $M.toGetParam(param));
						location.reload();
					}
				}
			);
		...
		function fnEndConsult() {
			...
			$M.goNextPageAjax(this_page + "/endConsult", $M.toGetParam(param) , {method : 'POST'},
				function(result) {
		    		if(result.success) {
		    			alert("상담종료가 완료되었습니다.");
		    			location.reload();
		...
		<div class="content-wrap">
			...
			<div class="form-group mt5">
				<table id="cust_info" class="table-border">
					...
					<tr>
						<th class="text-right">계약일자</th>
						<td>
							<div class="input-group">
								<input type="text" class="form-control border-right-0 calDate" id="doc_dt" name="doc_dt" value="${bean.doc_dt}" dateformat="yyyy-MM-dd" disabled="disabled">
							</div>
						</td>
						<th class="text-right">출하일자</th>
						<td>
							<div class="input-group">
								<input type="text" class="form-control border-right-0 calDate" id="out_dt" name="out_dt" value="${bean.out_dt}" dateFormat="yyyy-MM-dd" disabled="disabled">
							</div>
						</td>
					...
					<tr>
						...
						<th class="text-right">상담모델</th>
						<td>
							<div class="form-row inline-pd">
								<div class="col-10">
									<div class="input-group">
										<input type="text" class="form-control border-right-0 width140px essential-bg" id="machine_name" name="machine_name" ${bean.machine_plant_seq != '' ? 'readonly="readonly"' : '' } value="${bean.machine_name}" alt="모델명" readonly="readonly">
