...
// 			$M.setValue(frm, "cmd_str", $M.getArrStr(cmdArr, option));
			...
			$M.goNextPageAjax("/cust/cust010101/save", frm, {method: 'POST'},
				function (result) {
				if (result.success) {
					if (result.dupl_yn == "Y") {
		...
		function goModelInfoClick() {
			...
			var param = {
				...
				s_rental_consult_yn: "Y",
				s_own_org_code: $M.getValue("center_org_code")
			};
		...
		function fnSetCustInfo(data) {
			...
			// 신규(N) 아니면 기존 등급, 신규(N)이면 렌탈(R)
			var custGradeCd = data.cust_grade_cd;
			if ("N" == data.cust_grade_cd) {
				custGradeCd = "R"
			}
			$M.setValue("s_cust_grade_cd", custGradeCd);
			setCustGradeDesc(custGradeCd);
		...
		function fnSearchConsultMachine(data) {
			var param = {
				...
				, "s_consult_type_cd" : "03"
			};

			// $M.goNextPageAjax(this_page + "/search/custMachine", $M.toGetParam(param), {method: 'GET'},
			
			$M.goNextPageAjax("/cust/cust010101" + "/search/consultMachine", $M.toGetParam(param), {method: 'GET'},
					function (result) {
						if (result.success) {
...
<input type="hidden" name="consult_type_cd" value="03">
...
<!-- 										<button type="button" class="btn btn-icon btn-primary-gra" onclick="javascript:openSearchCustPanel('fnSetCustInfo')"><i class="material-iconssearch"></i></button> -->
						...
						<td>
							<div class="form-row inline-pd">
								...
								<div class="algin-item-center">
									...
									<div class="form-check form-check-inline mr3">
										<input class="form-check-input" type="radio" id="consult_type_cd_02" name="consult_type_cd_val" value="12" disabled="disabled">
										<label for="consult_type_cd_02" class="form-check-label">대차</label>
										<sapn id="parent_consult_machine_name"></sapn>
									</div>
									<div class="form-check form-check-inline mr3">
										<input class="form-check-input" type="radio" id="consult_type_cd_03" name="consult_type_cd_val" value="23" checked="checked" disabled="disabled">
								...
								<div class="col-2">
									<select class="form-control" name="s_cust_grade_cd" id="s_cust_grade_cd" onchange="javascript:setCustGradeDesc(this.options[this.selectedIndex].value);" disabled="disabled">
										<c:forEach items="${codeMap['CUST_GRADE']}" var="item">
											<option value="${item.code_value}" ${item.code_value eq 'R' ? 'selected="selected"' : ''} >${item.code_name} </option>
										</c:forEach>
									</select>
								</div>
							</div>
						</td>
						<th class="text-right  essential-item">담당지역</th>
						<td>
							<div class="form-row inline-pd">
								<div class="col width140px">
									<input type="text" class="form-control essential-bg" id="area_si" name="area_si" alt="담당지역" readonly="readonly" required="required">
								</div>
							</div>
						</td>
						<th class="text-right  essential-item">담당센터</th>
						<td>
							<div class="form-row inline-pd">
								<div class="col-3">
									<input type="text" class="form-control essential-bg" id="center_org_name" name="center_org_name" alt="담당센터" readonly="readonly" required="required">
								</div>
							</div>
						</td>
					</tr>
			...
			<div class="btn-group mt10">
				<div class="right">
					<button type="button" class="btn btn-info"  onclick="javascript:goRentalPage();">렌탈신청현황</button>
