<%@ page contentType="text/html;charset=utf-8" language="java"%><jsp:include page="/WEB-INF/jsp/common/commonForAll.jsp"/><%@ taglib prefix="c" uri="http://java.sun.com/jstl/core_rt" %><%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %><%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %><%@ taglib uri="http://www.springframework.org/tags" prefix="spring" %><%@ taglib uri="http://www.springframework.org/tags/form" prefix="form"%>
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 고객 > 고객현황 > 거래원장 > null > null
-- 작성자 : 성현우
-- 최초 작성일 : 2020-09-08 14:23:48
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	<jsp:include page="/WEB-INF/jsp/common/auiHeader.jsp"/>
	<script type="text/javascript">

		var auiGrid;
		var page = 1;
		var moreFlag = "N";
		var isLoading = false;

		$(document).ready(function () {
			createAUIGrid();
		});

		// 엔터키 이벤트
		function enter(fieldObj) {
			var field = ["s_cust_name", "s_breg_name", "s_deposit_name", "s_hp_no", "s_breg_no", "s_breg_rep_name"];
			$.each(field, function () {
				if (fieldObj.name == this) {
					goSearch();
				}
			});
		}

		function goSearch() {
			if($M.getValue("s_cust_name") == "" && $M.getValue("s_hp_no") == "" && $M.getValue("s_breg_name") == ""
					&& $M.getValue("s_breg_no") == "" && $M.getValue("s_deposit_name") == "" && $M.getValue("s_breg_rep_name") == "") {
				alert("검색조건 중 하나는 필수입력입니다.");
				return;
			}

			// 조회 버튼 눌렀을경우 1페이지로 초기화
			page = 1;
			moreFlag = "N";
			fnSearch(function (result) {
				AUIGrid.setGridData(auiGrid, result.list);
				$("#total_cnt").html(result.total_cnt);
				$("#curr_cnt").html(result.list.length);
				if (result.more_yn == 'Y') {
					moreFlag = "Y";
					page++;
				}
			});
		}

		// 조회
		function fnSearch(successFunc) {
			var frm = document.main_form;
			//validationcheck
			if ($M.validation(frm,
					{field: ["s_start_dt", "s_end_dt"]}) == false) {
				return;
			}

			if ($M.checkRangeByFieldName("s_start_dt", "s_end_dt", true) == false) {
				return;
			}

			var params = {
				"s_start_dt": $M.getValue("s_start_dt"),
				"s_end_dt": $M.getValue("s_end_dt"),
				"s_cust_name": $M.getValue("s_cust_name"),
				"s_deposit_name": $M.getValue("s_deposit_name"),
				"s_hp_no": $M.getValue("s_hp_no"),
				"s_breg_name": $M.getValue("s_breg_name"),
				"s_breg_no": $M.getValue("s_breg_no"),
				"s_breg_rep_name": $M.getValue("s_breg_rep_name"),
				"s_masking_yn": $M.getValue("s_masking_yn") == "Y" || ${page.add.POS_UNMASKING ne 'Y'} ? "Y" : "N",
				"page": page,
				"rows": $M.getValue("s_rows")
			}
			_fnAddSearchDt(params, 's_start_dt', 's_end_dt');
			$M.goNextPageAjax(this_page + "/search", $M.toGetParam(params), {method: 'GET'},
					function (result) {
						isLoading = false;
						if (result.success) {
							successFunc(result);
						}
					}
			);
		}

		// 스크롤 위치가 마지막과 일치한다면 추가 데이터 요청함
		function fnScollChangeHandelr(event) {
			if (event.position == event.maxPosition && moreFlag == "Y" && isLoading == false) {
				goMoreData();
			}
		}

		function goMoreData() {
			fnSearch(function (result) {
				result.more_yn == "N" ? moreFlag = "N" : page++;
				if (result.list.length > 0) {
					console.log(result.list);
					AUIGrid.appendData("#auiGrid", result.list);
					$("#curr_cnt").html(AUIGrid.getGridData(auiGrid).length);
				}
			});
		}

		// 엑셀 다운로드
		function fnDownloadExcel() {
			fnExportExcel(auiGrid, "거래원장");
		}

		function createAUIGrid() {
			var gridPros = {
				// Row번호 표시 여부
				showRowNumColumn: true,
			};

			var columnLayout = [
				{
					headerText: "최종거래일",
					dataField: "last_sale_dt",
					dataType: "date",
					formatString: "yy-mm-dd",
					width : "70",
					minWidth : "60",
				},
				{
					headerText: "고객명",
					dataField: "cust_name",
					style: "aui-center aui-popup",
					width : "130",
					minWidth : "120"
				},
				{
					headerText: "고객번호",
					dataField: "cust_no",
					visible: false
				},
				{
					headerText: "업체명",
					dataField: "breg_name",
					width : "130",
					minWidth : "120"
				},
				{
					headerText: "대표자명",
					dataField: "breg_rep_name",
					width : "130",
					minWidth : "120"
				},
				{
					headerText: "입금자명",
					dataField: "deposit_name",
					width : "130",
					minWidth : "120"
				},
				{
					headerText: "주소",
					dataField: "addr",
					style: "aui-left",
					width : "220",
					minWidth : "210"
				},
				{
					headerText: "잔액",
					dataField: "in_amt",
					dataType: "numeric",
					formatString: "#,##0",
					labelFunction : function(rowIndex, columnIndex, value, headerText, item) {
						return  (value == "" || value == null) ? "" : $M.setComma(value);
					},
					style: "aui-right",
					width : "100",
					minWidth : "90"
				},
				{
					headerText: "수주",
					dataField: "part_sale_no",
					style: "aui-center aui-popup",
					width : "80",
					minWidth : "70",
					labelFunction : function(rowIndex, columnIndex, value, headerText, item) {
						return value.substring(4, 16);
					}
				},
				{
					headerText: "정비",
					dataField: "job_report_no",
					style: "aui-center aui-popup",
					width : "100",
					minWidth : "90",
					labelFunction : function(rowIndex, columnIndex, value, headerText, item) {
						return value.substring(4, 16);
					}
				},
				{
					headerText: "렌탈",
					dataField: "rental_doc_no",
					style: "aui-center aui-popup",
					width : "100",
					minWidth : "90",
					labelFunction : function(rowIndex, columnIndex, value, headerText, item) {
						return value.substring(4, 16);
					}
				},
				{
					headerText: "휴대폰",
					dataField: "hp_no",
					width : "110",
					minWidth : "100"
				},
				{
					headerText: "사업자번호",
					dataField: "breg_no",
					width : "110",
					minWidth : "100"
				}
			];

			// 실제로 #grid_wrap에 그리드 생성
			auiGrid = AUIGrid.create("#auiGrid", columnLayout, gridPros);
			// 그리드 갱신
			AUIGrid.setGridData(auiGrid, []);

			AUIGrid.bind(auiGrid, "cellClick", function (event) {
				var params = {};
				var popupOption = "";
				switch (event.dataField) {
					case "cust_name" :
						params = {
							"s_cust_no": event.item.cust_no,
// 							"s_ledger_yn" : "Y",
							"s_start_dt": $M.getValue("s_start_dt"),
							"s_end_dt": $M.getValue("s_end_dt")
						};
						openDealLedgerPanel($M.toGetParam(params));
						break;
					case "part_sale_no" :
						if(event.item.part_sale_no != "") {
						params = {
							"part_sale_no": event.item.part_sale_no
						};
							$M.goNextPage('/cust/cust0201p01', $M.toGetParam(params), {popupStatus: popupOption});
						}
						break;
					case "job_report_no" :
						if(event.item.job_report_no != "") {
							params = {
								"s_job_report_no": event.item.job_report_no
							};
							$M.goNextPage('/serv/serv0101p01', $M.toGetParam(params), {popupStatus: popupOption});
						}
						break;
					case "rental_doc_no" :
						if(event.item.rental_doc_no != "") {
							params = {
								"rental_doc_no": event.item.rental_doc_no
							};
							popupOption = "scrollbars=no, resizable=yes, menubar=no, toolbar=no, location=no, directories=no, status=no, fullscreen=no, width=1500, height=730, left=0, top=0";
							$M.goNextPage('/rent/rent0101p01', $M.toGetParam(params), {popupStatus: popupOption});
						}
						break;
				}
			});

			AUIGrid.bind(auiGrid, "vScrollChange", fnScollChangeHandelr);
		}
	</script>
</head>
<body>
<form id="main_form" name="main_form">
	<div class="layout-box">
		<!-- contents 전체 영역 -->
		<div class="content-wrap">
			<div class="content-box">
				<!-- 메인 타이틀 -->
				<div class="main-title">
					<jsp:include page="/WEB-INF/jsp/common/menuNavi.jsp"/>
				</div>
				<!-- /메인 타이틀 -->
				<div class="contents">
					<!-- 기본 -->
					<div class="search-wrap">
						<table class="table">
							<colgroup>
								<col width="70px">
								<col width="280px">
								<col width="40px">
								<col width="120px">
								<col width="50px">
								<col width="120px">
								<col width="60px">
								<col width="120px">
								<col width="60px">
								<col width="120px">
								<col width="70px">
								<col width="140px">
								<col width="70px">
								<col width="140px">
								<col width="">
							</colgroup>
							<tbody>
							<tr>
								<th>최종거래일</th>
								<td>
									<div class="row mg0">
										<div class="col-5">
											<div class="input-group dev_nf">
												<input type="text" class="form-control border-right-0 calDate" id="s_start_dt" name="s_start_dt" dateFormat="yyyy-MM-dd" value="${searchDtMap.s_start_dt}" alt="조회 시작일">
											</div>
										</div>
										<div class="col-auto">~</div>
										<div class="col-5">
											<div class="input-group dev_nf">
												<input type="text" class="form-control border-right-0 calDate" id="s_end_dt" name="s_end_dt" dateFormat="yyyy-MM-dd" value="${searchDtMap.s_end_dt}" alt="조회 종료일">
											</div>
										</div>

										<!-- <details data-popover="up">

                                       </details> -->
										<jsp:include page="/WEB-INF/jsp/common/searchDtType.jsp">
											<jsp:param name="st_field_name" value="s_start_dt"/>
											<jsp:param name="ed_field_name" value="s_end_dt"/>
											<jsp:param name="click_exec_yn" value="N"/>
											<jsp:param name="exec_func_name" value=""/>
										</jsp:include>
									</div>
								</td>
								<th>고객명</th>
								<td>
									<input type="text" class="form-control" id="s_cust_name" name="s_cust_name">
								</td>
								<th>업체명</th>
								<td>
									<input type="text" class="form-control" id="s_breg_name" name="s_breg_name">
								</td>
								<th>입금자명</th>
								<td>
									<input type="text" class="form-control" id="s_deposit_name" name="s_deposit_name">
								</td>
								<th>휴대폰</th>
								<td>
									<input type="text" class="form-control" id="s_hp_no" name="s_hp_no" placeholder="-없이 숫자만" datatype="int">
								</td>
								<th>사업자번호</th>
								<td>
									<input type="text" class="form-control" id="s_breg_no" name="s_breg_no" placeholder="-없이 숫자만" datatype="int">
								</td>
								<th>대표자명</th>
								<td>
									<input type="text" class="form-control" id="s_breg_rep_name" name="s_breg_rep_name">
								</td>
								<td class="">
									<button type="button" class="btn btn-important" style="width: 50px;" onclick="javascript:goSearch()">조회</button>
								</td>
							</tr>
							</tbody>
						</table>
					</div>
					<!-- /기본 -->
					<!-- 그리드 타이틀, 컨트롤 영역 -->
					<div class="title-wrap mt10">
						<h4>거래내역</h4>
						<div class="btn-group">
							<div class="right">
								<c:if test="${page.add.POS_UNMASKING eq 'Y'}">
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="checkbox" id="s_masking_yn" name="s_masking_yn"<c:if test="${page.masking_default_yn eq 'Y'}" > checked</c:if> value="Y" >
										<label class="form-check-input" for="s_masking_yn">마스킹 적용</label>
									</div>
								</c:if>
								<jsp:include page="/WEB-INF/jsp/common/buttonAuth.jsp"><jsp:param name="pos" value="TOP_R"/></jsp:include>
							</div>
						</div>
					</div>
					<!-- /그리드 타이틀, 컨트롤 영역 -->
					<div id="auiGrid" style="margin-top: 5px; height: 555px;"></div>
					<!-- 그리드 서머리, 컨트롤 영역 -->
					<div class="btn-group mt5">
						<div class="left">
							<jsp:include page="/WEB-INF/jsp/common/pagingFooter.jsp"/>
						</div>
					</div>
					<!-- /그리드 서머리, 컨트롤 영역 -->
				</div>
			</div>
			<jsp:include page="/WEB-INF/jsp/common/footer.jsp"/>
		</div>
		<!-- /contents 전체 영역 -->
	</div>
</form>
</body>
</html>