...
<%------------------------------------------------------------------------------------------------------------------
-- 업   무 : 영업 > 장비관리 > 장비입고-LC Open 선적 > null > 차대번호 일괄등록(컨테이너)
-- 작성자 : 황빛찬
-- 최초 작성일 : 2022-09-14 17:41:08
------------------------------------------------------------------------------------------------------------------%>
<!DOCTYPE html>
<html>
<head>
	...
	function createAUIGrid() {
		...
		var columnLayout = [
			...
			{ 
				...
				editRenderer : {				
					...
					list : containerList,
					keyField : "container_name",
					valueField : "container_name"
				},
				labelFunction : function(rowIndex, columnIndex, value){
					// for(var i=0; i<containerList.length; i++){
					// 	if(value == containerList[i].container_name){
		...
		AUIGrid.bind(auiGrid, "cellEditEnd", function (event) {
			...
			if (event.dataField == "container_name") {
				...
				for (var i = 0; i < containerList.length; i++) {
					if (containerList[i].container_name == event.item.container_name) {
						AUIGrid.setCellValue(auiGrid, event.rowIndex, "container_seq", containerList[i].container_seq);
						containerSeq = containerList[i].container_seq;
					}
				}
				...
				if (containerSeq == undefined) {
					alert((event.rowIndex + 1) + "행에 등록되지 않은 컨테이너가 입력되었습니다.");
					AUIGrid.setCellValue(auiGrid, event.rowIndex, "container_name", "");
					AUIGrid.updateRow(auiGrid, {"container_name" : ""}, event.rowIndex);
					return;
				} else {
					var param = {
