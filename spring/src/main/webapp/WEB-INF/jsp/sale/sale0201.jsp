...
// 			$M.setValue("s_start_dt", $M.addMonths($M.toDate(now), -1));
		...
		function createAUIGrid() {
			...
			var columnLayout = [
				{
					dataField : "origin_machine_order_no",
					visible : false
				},
				{
					headerText : "등록일", 
			...
			AUIGrid.bind(auiGrid, "cellClick", function(event) {
				// 보낼 데이터
				var param = {
					machine_order_no : event.item.origin_machine_order_no
				};
...
// 					var popupOption = "scrollbars=no, resizable-1, menubar=no, toolbar=no, location=no, directories=no, status=no, fullscreen=no, width=1650, height=910, left=0, top=0";
		...
		function goSearch() {		
			var param = {
					s_machine_order_no : $M.getValue("s_machine_order_no"),
// 					s_cust_no : $M.getValue("s_cust_no"),
		...
		function fnDownloadExcel() {
			var exportProps = {
		  	...
		  	fnExportExcel(auiGrid, "장비생산발주", exportProps);
		}
		
		// 생산발주등록 페이지 이동
		function goNew() {
			$M.goNextPage("/sale/sale020101");
		}
								...
								<tr>
									...
									<td>	
										<div class="icon-btn-cancel-wrap">
											<input type="text" class="form-control" id="s_machine_order_no" name="s_machine_order_no">
										</div>
									</td>
									<th>발주처</th>
									<td>			
